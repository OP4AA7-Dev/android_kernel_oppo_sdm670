...
#endif /* CONFIG_MACH_OPLUS */

#ifdef CONFIG_MACH_OPLUS
#ifdef CONFIG_OPPO_KEVENT_UPLOAD
/*xiang.fei@PSW.MM.AudioDriver.Codec, 2019/02/21, Add for kevent log*/
#include <asoc/oppo_mm_audio_kevent.h>

bool g_event_id_dbmdx_card_fail = false;
bool g_event_id_dbmdx_spi_fail = false;
bool g_event_id_dbmdx_chip_id_fail = false;
bool g_event_id_dbmdx_fw_ver_fail = false;
...
#define MAX_MP_KFIFO_BUFFER_SIZE		MAX_KFIFO_BUFFER_SIZE
				     ...
				     enum dbmdx_microphone_mode mode)
{
	...
	unsigned int new_mp_kfifo_size;
	...
	new_mp_kfifo_size = p->multipurpose_samples_kfifo_buf_size;
	...
	if (p->multipurpose_kfifo_in_use &&
		(new_mp_kfifo_size != p->multipurpose_samples_kfifo_buf_size)) {
		p->multipurpose_samples_kfifo_buf_size = new_mp_kfifo_size;
		kfifo_init(&p->multipurpose_samples_kfifo,
			p->multipurpose_samples_kfifo_buf,
			new_mp_kfifo_size);
	}
out:
	return ret;
...
static int dbmdx_config_va_mode(struct dbmdx_private *p)
{
	...
	if (ret < 0) {
		...
		#ifdef CONFIG_OPPO_KEVENT_UPLOAD
		/*xiang.fei@PSW.MM.AudioDriver.Codec, 2019/02/21, Add for kevent log*/
		g_event_id_dbmdx_chip_id_fail = true;
		#endif /* CONFIG_OPPO_KEVENT_UPLOAD */
		#endif /* CONFIG_MACH_OPLUS */
	...
	if (ret < 0) {
		...
		#ifdef CONFIG_OPPO_KEVENT_UPLOAD
		/*xiang.fei@PSW.MM.AudioDriver.Codec, 2019/02/21, Add for kevent log*/
		g_event_id_dbmdx_fw_ver_fail = true;
		#endif /* CONFIG_OPPO_KEVENT_UPLOAD */
		#endif /* CONFIG_MACH_OPLUS */
	...
	if (p->pdata->boot_options & DBMDX_BOOT_OPT_FW_INCLUDES_PRI_AMODEL) {
		dev_info(p->dev, "%s: FW includes Acoustic Model\n", __func__);
		p->va_flags.a_model_downloaded_to_fw = 1;
		p->va_flags.amodel_len = 0x1234;
	}
					...
					bool to_load_from_memory)
{
	...
	if (ret) {
		dev_err(p->dev, "%s: sending primary amodel failed\n",
			__func__);
		ret = -EIO;
		goto out_finish_loading;
	}
	...
	if (ret) {
		dev_err(p->dev, "%s: sending secondary amodel failed\n",
			__func__);
		ret = -EIO;
		goto out_finish_loading;
	}
...
static int dbmdx_va_amodel_update(struct dbmdx_private *p, int val)
{
	...
	if (p->pdata->boot_options & DBMDX_BOOT_OPT_FW_INCLUDES_PRI_AMODEL &&
		sv_model_selected) {
		dev_info(p->dev,
			"%s: FW includes embedded Amodel, do not reload\n",
				__func__);
		do_not_reload_model = true;
...
/*xiang.fei@PSW.MM.AudioDriver.Codec, 2018/12/17, Modify for coverity*/
	unsigned long val;
#else
	long val;
#endif /* CONFIG_MACH_OPLUS */
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (p->pdata->feature_va) {
		...
		off += snprintf(buf + off, PAGE_SIZE - off,
				"\tVA Advanced options: 0x%x\n",
					p->pdata->va_advanced_options);
		off += snprintf(buf + off, PAGE_SIZE - off,
				"\tBuffering options: 0x%x\n",
				p->buffering_options);

		if (p->buffering_options & DBMDX_BUFFERING_OPT_SPLIT_PASSPHRASE)
			off += snprintf(buf + off, PAGE_SIZE - off,
			"\tPost Det. Margin: %u ms, Pre Det. Margin: %u ms\n",
					p->post_backlog_time_ms,
					p->pre_backlog_time_ms);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (p->pdata->feature_va) {
		...
		if (p->multipurpose_kfifo_in_use)
			off += snprintf(buf + off, PAGE_SIZE - off,
					"\tVA Multipurpose KFIFO size:\t%d\n",
					p->multipurpose_samples_kfifo_buf_size);
		off += snprintf(buf + off, PAGE_SIZE - off,
					"\tVA Detection mode:\t%s\n",
			p->va_detection_mode == DETECTION_MODE_PHRASE ?
				"PASSPHRASE" :
...
#define DBMDX_START_PCM_TIMING_PROFILING	1

#if DBMDX_START_PCM_TIMING_PROFILING
#include <linux/ktime.h>

#define DBMDX_START_PCM_TIME_WARNING		250
...
out:
	...
	dev_info(p->dev, "%s: Elapsed time: %ld ms",
		__func__,
		1000 * (tend.tv_sec - tstart.tv_sec) +
		(tend.tv_usec - tstart.tv_usec) / 1000);

	if ((1000 * (tend.tv_sec - tstart.tv_sec) +
		(tend.tv_usec - tstart.tv_usec) / 1000) >
					DBMDX_START_PCM_TIME_WARNING)
		dev_info(p->dev,
			"%s:PCM Start took too much time: %ld ms,limit: %d ms",
			__func__,
			1000 * (tend.tv_sec - tstart.tv_sec) +
			(tend.tv_usec - tstart.tv_usec) / 1000,
			DBMDX_START_PCM_TIME_WARNING);
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	#ifdef CONFIG_MACH_OPLUS
	#ifdef CONFIG_OPPO_KEVENT_UPLOAD
	/*xiang.fei@PSW.MM.AudioDriver.Codec, 2019/02/13, Add for kevent log*/
	...
	#ifdef CONFIG_OPPO_KEVENT_UPLOAD
	/*xiang.fei@PSW.MM.AudioDriver.Codec, 2019/02/13, Add for kevent log*/
	if (g_event_id_dbmdx_card_fail) {
		g_event_id_dbmdx_card_fail = false;
		scnprintf(payload, sizeof(payload), "NULL$$EventID@@%d$$dbmdx_card_fail",
			OPPO_MM_AUDIO_EVENT_ID_DBMDX_CARD_FAIL);
		upload_mm_audio_kevent_data(payload);
		dev_info(p->dev, "%s: OPPO_MM_AUDIO_EVENT_ID_DBMDX_CARD_FAIL\n", __func__);
	}

	if (g_event_id_dbmdx_spi_fail) {
		g_event_id_dbmdx_spi_fail = false;
		scnprintf(payload, sizeof(payload), "NULL$$EventID@@%d$$dbmdx_spi_fail",
			OPPO_MM_AUDIO_EVENT_ID_DBMDX_SPI_FAIL);
		upload_mm_audio_kevent_data(payload);
		dev_info(p->dev, "%s: OPPO_MM_AUDIO_EVENT_ID_DBMDX_SPI_FAIL\n", __func__);
	}

	if (g_event_id_dbmdx_chip_id_fail) {
		g_event_id_dbmdx_chip_id_fail = false;
		scnprintf(payload, sizeof(payload), "NULL$$EventID@@%d$$dbmdx_chip_id_fail",
			OPPO_MM_AUDIO_EVENT_ID_DBMDX_CHIP_ID_FAIL);
		upload_mm_audio_kevent_data(payload);
		dev_info(p->dev, "%s: OPPO_MM_AUDIO_EVENT_ID_DBMDX_CHIP_ID_FAIL\n", __func__);
	}

	if (g_event_id_dbmdx_fw_ver_fail) {
		g_event_id_dbmdx_fw_ver_fail = false;
		scnprintf(payload, sizeof(payload), "NULL$$EventID@@%d$$dbmdx_fw_ver_fail",
			OPPO_MM_AUDIO_EVENT_ID_DBMDX_FW_VER_FAIL);
		upload_mm_audio_kevent_data(payload);
		dev_info(p->dev, "%s: OPPO_MM_AUDIO_EVENT_ID_DBMDX_FW_VER_FAIL\n", __func__);
	}
	#endif /* CONFIG_OPPO_KEVENT_UPLOAD */
	#endif /* CONFIG_MACH_OPLUS */
...
#ifdef EXTERNAL_SOC_AMODEL_LOADING_ENABLED
#ifndef SOC_BYTES_EXT_HAS_KCONTROL_FIELD
...
static int dbmdx_send_uevent(struct dbmdx_private *p, char *uevent_buf)
{
	int ret = 0;
	...
	ret = kobject_uevent_env(&p->dev->kobj, KOBJ_CHANGE, (char **)envp);
					...
					bool to_start_buffering)
{
	...
	bool send_detection_uevent = false;
	bool build_detection_uevent = false;
	int ret;
	...
	if (p->uevent_options & DBMDX_UEVENT_OPT_SEND_DETECTION_EVENT) {
		build_detection_uevent = true;
		if (!(p->uevent_options &
			DBMDX_UEVENT_OPT_SEND_AFTER_PASSPHRASE_READY))
			send_detection_uevent = true;
	} else {
	...
	if (to_start_buffering) {
		...
		if (p->multipurpose_kfifo_in_use && !send_detection_uevent)
			kfifo_reset(&p->multipurpose_samples_kfifo);
	...
	if (build_detection_uevent) {
		char *uevent_buf = p->uevent_buf;
#ifdef SV_FW_DETECTION_STATS
		...
		if (p->va_detection_mode == DETECTION_MODE_PHRASE) {
			...
			off = snprintf(uevent_buf, UEVENT_BUF_SIZE,
			...
			snprintf(uevent_buf, UEVENT_BUF_SIZE,
				"VOICE_WAKEUP EVENT_TYPE=Detection EVENT_ID=%d",
				event_id);
		}
#else
	...
	if (send_detection_uevent) {
		...
		p->va_flags.det_uevent_in_sv_work = true;
	}
...
static void dbmdx_uevent_work(struct work_struct *work)
{
	...
	if (ret < 0) {
		...
		#ifdef CONFIG_OPPO_KEVENT_UPLOAD
		/*xiang.fei@PSW.MM.AudioDriver.Codec, 2019/02/21, Add for kevent log*/
		scnprintf(payload, sizeof(payload), "NULL$$EventID@@%d$$dbmdx_detection_irq_fail",
			OPPO_MM_AUDIO_EVENT_ID_DBMDX_DETECTION_IRQ_FAIL);
		upload_mm_audio_kevent_data(payload);
		dev_info(p->dev, "%s: OPPO_MM_AUDIO_EVENT_ID_DBMDX_DETECTION_IRQ_FAIL\n", __func__);
		#endif /* CONFIG_OPPO_KEVENT_UPLOAD */
		#endif /* CONFIG_MACH_OPLUS */
...
int calc_sliding_window_pos(struct dbmdx_private *p,
				size_t cur_pos, size_t left_border,
				int right_border, size_t window_len,
				unsigned int sample_len,
				size_t *window_offset,
				size_t *window_len_in)
{
	size_t right_border_upd =
				(right_border <= 0) ?
						(UINT_MAX - 1) : right_border;
	size_t cur_pos_upd = cur_pos + window_len;

#ifdef SLIDING_WINDOW_DBG
	dev_dbg(p->dev, "%s CurPos: %zu\tLeft: %zu\tRight: %zu\tWindowLen: %zu\n",
		__func__, cur_pos, left_border, right_border_upd, window_len);
#endif

	if ((cur_pos % sample_len) || (left_border % sample_len) ||
		(right_border_upd % sample_len) || (window_len % sample_len)) {
		dev_err(p->dev,
			"%s: All params have to be mult, of sample_len: %u\n",
			__func__, sample_len);
		return -EINVAL;

	}

	if ((cur_pos_upd < left_border) || (cur_pos > right_border_upd)) {
		*window_len_in = 0;
		*window_offset = 0;
	} else {
		*window_len_in = window_len;
		*window_offset = 0;

		if (cur_pos < left_border) {
			*window_offset = window_len -
						(cur_pos_upd - left_border);
			*window_len_in -= *window_offset;
		}

		if (cur_pos_upd > right_border_upd)
			*window_len_in -= (cur_pos_upd - right_border_upd);
	}
...
static void dbmdx_sv_work(struct work_struct *work)
{
	...
	int retries = 0;
	size_t data_offset;
	bool split_passphrase_and_audio = false;
	bool duplicate_detection_buffer = false;
	bool use_mp_kfifo = false;
	size_t det_buf_start_point_in_bytes = 0;
	size_t mp_buf_start_point_in_bytes = 0;
	size_t mp_buf_end_point_in_bytes = 0;
	size_t total_bytes_detection_buf = 0;
	size_t total_bytes_mp_buf = 0;
	...
	if (p->va_detection_mode == DETECTION_MODE_DUAL) {
		dev_dbg(p->dev,	"%s:dual mode: ->voice_command mode\n",
			__func__);
		return;
	}
	...
	if (!(p->multipurpose_kfifo_in_use)) {
		split_passphrase_and_audio = false;
		duplicate_detection_buffer = false;
		use_mp_kfifo = false;
	} else if ((p->buffering_options &
				DBMDX_BUFFERING_OPT_SPLIT_PASSPHRASE) &&
			p->va_cur_backlog_length) {
		split_passphrase_and_audio = true;
		duplicate_detection_buffer = false;
		use_mp_kfifo = true;
	} else if (p->buffering_options &
				DBMDX_BUFFERING_OPT_DUPLICATE_BUFFER) {
		split_passphrase_and_audio = false;
		duplicate_detection_buffer = true;
		use_mp_kfifo = true;
	}
	...
	if (split_passphrase_and_audio) {

		size_t backlog_len_in_bytes = 0;
		size_t backlog_additional_bytes = 0;
		size_t audio_additional_bytes = 0;
		int num_of_channels = p->pdata->va_audio_channels;
		...
		backlog_len_in_bytes = nr_samples * 8 * bytes_per_sample;
		backlog_additional_bytes = (p->post_backlog_time_ms *
					p->pdata->va_buffering_pcm_rate *
					num_of_channels * bytes_per_sample) /
					1000;
		audio_additional_bytes = (p->pre_backlog_time_ms *
					p->pdata->va_buffering_pcm_rate *
					num_of_channels * bytes_per_sample) /
					1000;

		if (audio_additional_bytes < backlog_len_in_bytes)
			det_buf_start_point_in_bytes =
				backlog_len_in_bytes - audio_additional_bytes;


		mp_buf_start_point_in_bytes = 0;
		mp_buf_end_point_in_bytes =
				backlog_len_in_bytes + backlog_additional_bytes;

		dev_dbg(p->dev,
			"%s Det.Buf. Start: %zu\tMP.Start: %zu\tMp.End: %zu\n",
			__func__, det_buf_start_point_in_bytes,
			mp_buf_start_point_in_bytes, mp_buf_end_point_in_bytes);

	} else if (duplicate_detection_buffer) {
		mp_buf_start_point_in_bytes = det_buf_start_point_in_bytes;
		mp_buf_end_point_in_bytes = -1;
	} else if (p->va_flags.det_uevent_in_sv_work) {
	...
	do {
		...
		if (nr_samples) {
			...
			if (ret < 0) {
				...
				ret = calc_sliding_window_pos(p, total,
					det_buf_start_point_in_bytes, -1,
					bytes_to_read, bytes_per_sample,
					&cur_offset_bytes, &bytes_to_add);

				if (ret < 0) {
					dev_err(p->dev,
					"%s: Error in det. sl. window settings\n",
						__func__);
					p->unlock(p);
					break;
				}

				total_bytes_detection_buf += bytes_to_add;
				...
				if (bytes_to_add)
					...
					dbmdx_add_audio_samples_to_kfifo(p,
						...
						p->read_audio_buf +
						data_offset + cur_offset_bytes,
						bytes_to_add,
						p->detection_achannel_op);

				if (use_mp_kfifo) {
					ret = calc_sliding_window_pos(p, total,
						mp_buf_start_point_in_bytes,
						mp_buf_end_point_in_bytes,
						bytes_to_read, bytes_per_sample,
						&cur_offset_bytes,
						&bytes_to_add);
					if (ret < 0) {
						dev_err(p->dev,
							"%s: Error in mp. sl. window settings\n",
							__func__);
						p->unlock(p);
						break;
					}
					total_bytes_mp_buf += bytes_to_add;
				} else {
					bytes_to_add = 0;
				}
				...
				if (split_passphrase_and_audio &&
					p->va_flags.det_uevent_in_sv_work &&
					(total < mp_buf_end_point_in_bytes) &&
					(total + bytes_to_read >=
						mp_buf_end_point_in_bytes)) {
...
out:
	...
	if (p->pdata->send_uevent_after_buffering) {
		snprintf(p->uevent_buf, UEVENT_BUF_SIZE,
...
static int dbmdx_get_va_resources(struct dbmdx_private *p)
{
	...
	if (p->multipurpose_kfifo_in_use) {
		p->multipurpose_samples_kfifo_buf_size =
					MAX_MP_KFIFO_BUFFER_SIZE;

		p->multipurpose_samples_kfifo_buf =
			kmalloc(MAX_MP_KFIFO_BUFFER_SIZE, GFP_KERNEL);

		if (!p->multipurpose_samples_kfifo_buf) {
			ret = -ENOMEM;
			goto err_kfifo_free;
		}
		kfifo_init(&p->multipurpose_samples_kfifo,
...
err_kfifo_mp_free:
	if (p->multipurpose_kfifo_in_use)
		kfifo_free(&p->multipurpose_samples_kfifo);
err_kfifo_free:
...
static int dbmdx_parse_va_advanced_options(struct dbmdx_private *p)
{

	if (p->pdata->va_advanced_options &
				DBMDX_AVD_OPT_SPLIT_PASSPHRASE_BUFFER) {
		p->pre_backlog_time_ms =
				(p->pdata->va_passphrase_margins & 0xFFFF);
		p->post_backlog_time_ms =
		...
		p->multipurpose_kfifo_in_use = true;
	} else if (p->pdata->va_advanced_options &
				DBMDX_AVD_OPT_DUPLICATE_DETECTION_BUFFER) {
		p->pre_backlog_time_ms = 0;
		p->post_backlog_time_ms = 0;
		p->chdev_options = (DBMDX_CHDEV_OPT_DEV0_BLOCKING |
				DBMDX_CHDEV_OPT_DEV0_MP |
				DBMDX_CHDEV_OPT_CONT_BUFFERING_ON_DEV0_RELEASE);
		p->buffering_options = DBMDX_BUFFERING_OPT_DUPLICATE_BUFFER;
		p->multipurpose_kfifo_in_use = true;

	} else {
		p->pre_backlog_time_ms = 0;
		p->post_backlog_time_ms = 0;
		p->chdev_options = DBMDX_CHDEV_OPT_DEFAULT;
		p->buffering_options = DBMDX_BUFFERING_OPT_DEFAULT;
		p->multipurpose_kfifo_in_use = false;

	}

	if ((p->pdata->va_advanced_options &
				DBMDX_AVD_OPT_SEND_DETECTION_UEVENT) ||
		p->pdata->send_uevent_on_detection)
		p->uevent_options |= DBMDX_UEVENT_OPT_SEND_DETECTION_EVENT;

	if ((p->pdata->va_advanced_options &
				DBMDX_AVD_OPT_SEND_BUFFERING_DONE_UEVENT) ||
		p->pdata->send_uevent_after_buffering)
		p->uevent_options |= DBMDX_UEVENT_OPT_SEND_BUFFERING_DONE_EVENT;

	if ((p->pdata->va_advanced_options &
			DBMDX_AVD_OPT_SEND_DET_UEVENT_AFTER_PASSPHRASE) &&
		(p->pdata->va_advanced_options &
				DBMDX_AVD_OPT_SPLIT_PASSPHRASE_BUFFER))
		p->uevent_options |=
				DBMDX_UEVENT_OPT_SEND_AFTER_PASSPHRASE_READY;

	dev_info(p->dev, "%s: Advanced options: 0x%x\n",
				__func__, p->pdata->va_advanced_options);
	dev_info(p->dev, "%s: Buffering options: 0x%x\n",
				__func__, p->buffering_options);

	if (p->buffering_options & DBMDX_BUFFERING_OPT_SPLIT_PASSPHRASE)
		dev_info(p->dev,
			"%s: Post Det. Margin: %u ms, Pre Det. Margin: %u ms\n",
			__func__, p->post_backlog_time_ms,
			p->pre_backlog_time_ms);

	dev_info(p->dev, "%s: Uevent options: 0x%x\n",
				__func__, p->uevent_options);
	dev_info(p->dev, "%s: Ch.Dev options: 0x%x\n",
				__func__, p->chdev_options);
...
#ifdef CONFIG_SND_SOC_DBMDX_MODULE
		...
		struct dbmdx_private *p)
{
	...
	if (ret != 0) {
		dev_info(dev,
			"%s: no va_advanced_options def. in device-tree\n",
			__func__);
		p->pdata->va_advanced_options = DBMDX_AVD_OPT_DEFAULT;
	} else {
		dev_info(dev,
		"%s: using va_advanced_options of 0x%x from dev-tree\n",
			 __func__, p->pdata->va_advanced_options);
	}

	ret = of_property_read_u32(np,
				   "va_passphrase_margins",
				   &p->pdata->va_passphrase_margins);
	if (ret != 0) {
		dev_info(dev,
			"%s: no va_passphrase_margins def. in device-tree\n",
			__func__);
		p->pdata->va_passphrase_margins = 0;
	} else {
		dev_info(dev,
		"%s: using va_passphrase_margins of 0x%x from dev.-tree\n",
			 __func__, p->pdata->va_passphrase_margins);
	}

	ret = dbmdx_parse_va_advanced_options(p);
	if (ret)
		goto out_free_va_resources;
		...
		struct dbmdx_private *p)
{
	...
	if (pdata->feature_va) {
		...
		dev_info(dev, "%s: using va_advanced_options of 0x%x\n",
			 __func__, pdata->va_advanced_options);

		dev_info(dev, "%s: using va_passphrase_margins of 0x%x\n",
			 __func__, pdata->va_passphrase_margins);
...
static int __init dbmdx_modinit(void)
{
	...
	if (ret != 0) {
		pr_err("%s: Failed to register DBMDX Driver: %d\n",
		       __func__, ret);
		return ret;
	}
	...
	if (ret != 0) {
		pr_err("%s: Failed to register DBMDX SND Driver: %d\n",
		       __func__, ret);
	...
	if (ret != 0) {
		pr_err("%s: Failed to register DBMDX PCM Driver: %d\n",
		       __func__, ret);
		board_dbmdx_snd_exit();
		platform_driver_unregister(&dbmdx_platform_driver);
		return ret;
