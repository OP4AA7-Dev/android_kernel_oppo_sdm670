 ...
 * History :
   <version>   <time>      <author>  <desc>
    ...
    2.0         2015-04-13  hantong   modify as platform device  to support diffrent configure in
 dts
*/
...
struct devinfo_data {
    ...
    struct pinctrl_state* Sboard_gpio_pullup;
...
static int device_seq_show(struct seq_file* s, void* v) {
    ...
    if (info) {
        seq_printf(s, "Device version:\t\t%s\nDevice manufacture:\t\t%s\n", info->version,
                   info->manufacture);
        if (info->fw_path) seq_printf(s, "Device fw_path:\t\t%s\n", info->fw_path);
    }
    return 0;
}
...
static int get_hw_opreator_version(struct devinfo_data* devinfo_data) {
    ...
    devinfo_data->hw_operator_gpio_sleep =
            pinctrl_lookup_state(devinfo_data->pinctrl, "hw_operator_gpio_sleep");
    ...
    if (!IS_ERR_OR_NULL(devinfo_data->pinctrl)) {
        ...
        if (!IS_ERR_OR_NULL(devinfo_data->sub_id_active)) {
            ...
            if (gpio_is_valid(devinfo_data->sub_hw_id1) &&
                gpio_is_valid(devinfo_data->sub_hw_id2)) {
                sub_id1 = gpio_get_value(devinfo_data->sub_hw_id1);
                sub_id2 = gpio_get_value(devinfo_data->sub_hw_id2);
            }
            devinfo_data->sub_id_sleep =
                    pinctrl_lookup_state(devinfo_data->pinctrl, "sub_id_sleep");
...
sub_mainboard_set:
    ...
    switch (get_project()) {
        ...
        case OPPO_18085:
            ...
            if ((sub_mainboard_volt > 0 && sub_mainboard_volt < 200) &&
                (operator == OPERATOR_ALL_CHINA_CARRIER || operator == OPERATOR_CHINA_MOBILE) &&
                (pcbVersion == HW_VERSION__10)) {  // EVT1 china
                snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "rf-match");
            } else if ((sub_mainboard_volt > 250 && sub_mainboard_volt < 400) &&
                       (operator == OPERATOR_FOREIGN) &&
                       (pcbVersion == HW_VERSION__11 ||
                        pcbVersion == HW_VERSION__12)) {  // EVT2 foreign and DVT
                snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "rf-match");
            } else if ((sub_mainboard_volt > 500 && sub_mainboard_volt < 650) &&
                       (operator == OPERATOR_FOREIGN) &&
                       (pcbVersion == HW_VERSION__10)) {  // EVT1 foreign
                snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "rf-match");
            } else if ((sub_mainboard_volt > 850 && sub_mainboard_volt < 1000) &&
                       (operator == OPERATOR_FOREIGN) &&
                       (pcbVersion == HW_VERSION__13)) {  // PVT foreign
                snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "rf-match");
            } else if ((sub_mainboard_volt > 1650) &&
                       (operator == OPERATOR_ALL_CHINA_CARRIER ||
                        operator == OPERATOR_CHINA_MOBILE) &&
                       (pcbVersion == HW_VERSION__11 || pcbVersion == HW_VERSION__12 ||
                        pcbVersion == HW_VERSION__13)) {  // EVT2 china, DVT and PVT
                snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "rf-match");
            } else {
        ...
        case OPPO_18181:
            pr_err("sub_mainboard_volt: %d\n", sub_mainboard_volt);
            if ((sub_mainboard_volt > 0 && sub_mainboard_volt < 300) &&
                (operator == OPERATOR_ALL_CHINA_CARRIER || operator == OPERATOR_CHINA_MOBILE)) {
                snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "rf-match");
            } else if ((sub_mainboard_volt > 300 && sub_mainboard_volt < 700) &&
                       (operator == OPERATOR_FOREIGN || operator == OPERATOR_FOREIGN_EUROPE)) {
                snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "rf-match");
            } else {
                snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "rf-unmatch");
            }
        ...
        case OPPO_18041:
            ...
            break;
        default:
            snprintf(mainboard_info.manufacture, INFO_BUF_LEN, "%d-%d", get_project(),
...
static void RF_resource_verify(struct devinfo_data* devinfo_data) {
    ...
    devinfo_data->Sboard_gpio_pullup =
            pinctrl_lookup_state(devinfo_data->pinctrl, "Sboard_gpio_pullup");
    if (IS_ERR_OR_NULL(devinfo_data->Sboard_gpio_pullup)) {
        pr_err("%s:%d Failed to get the Sboard gpio pinctrl handle\n", __func__, __LINE__);
        goto get_info_error;
    }

    devinfo_data->Sboard_gpio_pulldown =
            pinctrl_lookup_state(devinfo_data->pinctrl, "Sboard_gpio_pulldown");
    ...
    switch (get_project()) {
        case OPPO_18181:
            if (gpio136_stu == 0 && (operator_name == OPERATOR_ALL_CHINA_CARRIER ||
            ...
            } else if (gpio136_stu == 1 && (operator_name == OPERATOR_FOREIGN ||
                                            operator_name == OPERATOR_FOREIGN_EUROPE)) {
                snprintf(rf_resource_info.manufacture, INFO_BUF_LEN, "Sboard-match");
                snprintf(rf_resource_info.version, INFO_BUF_LEN, "Qcom");
            } else {
...
static void wlan_resource_verify(struct devinfo_data* devinfo_data) {
    ...
    devinfo_data->hw_wlan_gpio_sleep =
            pinctrl_lookup_state(devinfo_data->pinctrl, "hw_wlan_gpio_sleep");
...
static ssize_t fork_para_monitor_read_proc(struct file* file, char __user* buf, size_t count,
                                           loff_t* off) {
    char page[256] = {0};
    int ret = 0;
    // ret = snprintf(page, 255, " times:%d\n father pid:%d\n child pid:%d\n", happend_times,
    // fork_pid_father, fork_pid_child);
...
static ssize_t mainboard_resource_read_proc(struct file* file, char __user* buf, size_t count,
                                            loff_t* off) {
    char page[256] = {0};
    int len = 0;
