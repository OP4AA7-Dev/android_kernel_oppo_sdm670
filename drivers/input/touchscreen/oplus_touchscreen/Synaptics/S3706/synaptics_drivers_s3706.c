...
static int synaptics_get_touch_points(void* chip_data, struct point_info* points, int max_num) {
    ...
    ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F12_2D_DATA_BASE,
...
static void synaptics_health_zone_init(struct chip_data_s3706* chip_info) {
    ...
    if (1 == ret) {
        ...
        l_tmp = touch_i2c_read_byte(chip_info->client,
                                    chip_info->reg_info.F51_CUSTOM_QUERY_BASE + 0x05);
        h_tmp = touch_i2c_read_byte(chip_info->client,
                                    chip_info->reg_info.F51_CUSTOM_QUERY_BASE + 0x06);
        ...
        TPD_INFO("l_tmp = 0x%x h_tmp = 0x%x chip_info->health_zone.loglength_addr = 0x%x \n", l_tmp,
                 h_tmp, chip_info->health_zone.loglength_addr);
    }
...
static int synaptics_get_chip_info(void* chip_data) {
    ...
    TPD_INFO(
            "F12_2D_QUERY_BASE = 0x%x \n\
                        F12_2D_CMD_BASE        = 0x%x \n\
                        F12_2D_CTRL_BASE   = 0x%x \n\
                        F12_2D_DATA_BASE   = 0x%x \n",
            reg_info->F12_2D_QUERY_BASE, reg_info->F12_2D_CMD_BASE, reg_info->F12_2D_CTRL_BASE,
            reg_info->F12_2D_DATA_BASE);
    ...
    TPD_INFO(
            "F01_RMI_QUERY_BASE = 0x%x \n\
                        F01_RMI_CMD_BASE        = 0x%x \n\
                        F01_RMI_CTRL_BASE   = 0x%x \n\
                        F01_RMI_DATA_BASE   = 0x%x \n",
            reg_info->F01_RMI_QUERY_BASE, reg_info->F01_RMI_CMD_BASE, reg_info->F01_RMI_CTRL_BASE,
            reg_info->F01_RMI_DATA_BASE);
    ...
    reg_info->F51_CUSTOM_DATA =
            reg_info->F51_CUSTOM_DATA_BASE; /*Roland, where is gesture type register*/
    ...
    reg_info->F51_CUSTOM_CTRL04_08 =
            reg_info->F51_CUSTOM_CTRL_BASE + 0x10; /*switch for corner mode*/
...
static uint8_t synaptics_get_noise_level(struct chip_data_s3706* chip_info) {
    ...
    touch_i2c_write_byte(chip_info->client, 0xff, 0x0);
    ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F12_2D_CTRL11, 13,
                               &(game_buffer[0]));
    if (ret < 0) {
...
static fw_check_state synaptics_fw_check(void* chip_data, struct resolution_info* resolution_info,
    ...
    TPD_INFO("max_x = %d, max_y = %d, max_x_ic = %d, max_y_ic = %d\n", resolution_info->max_x,
             resolution_info->max_y, max_x_ic, max_y_ic);
    ...
    if (chip_info->report_120hz_support) {
        ...
        TPD_INFO("%s: default_nosie_level = 0x%x, default_lock_point_level = 0x%x .\n", __func__,
                 chip_info->default_nosie_level, chip_info->default_lock_point_level);
    }
...
static int synaptics_enable_interrupt(struct chip_data_s3706* chip_info, bool enable) {
    ...
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F01_RMI_CTRL01,
                               abs_status_int);
    if (ret < 0) {
...
static int synaptics_configuration_init(struct chip_data_s3706* chip_info, bool config) {
    ...
    if (config) {  // configed  && out of sleep mode
        ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F01_RMI_CTRL00,
                                   (ret & 0xf8) | 0x80);
        if (ret < 0) {
            TPD_INFO("%s failed for mode select\n", __func__);
            return -1;
        }
        touch_i2c_read_block(chip_info->client, chip_info->reg_info.F12_2D_CTRL20, 3,
                             &(ctrl_buf[0x0]));
        if (ctrl_buf[2] & 0x02) {
            ...
            touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL13,
                                 retval & 0xFE);
            retval = touch_i2c_read_byte(chip_info->client,
                                         chip_info->reg_info.F51_CUSTOM_CTRL00 + 3);
            if (retval != 0) {
                touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL00 + 3,
                                     0x00);
                touch_i2c_write_byte(chip_info->client, 0xff, 0x01); /* page 1*/
                touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                                     0x04); /* force update*/
...
static int synaptics_glove_mode_enable(struct chip_data_s3706* chip_info, bool enable) {
    ...
    if (enable) {
        ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F12_2D_CTRL23,
                                   ret | 0x20);
    } else {
        ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F12_2D_CTRL23,
                                   ret & 0xdf);
...
static void synaptics_enable_fingerprint_underscreen(void* chip_data, uint32_t enable) {
    ...
    if (ret != enable)
        ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL20,
                                   enable); /*open gesture for fingerprint under screen*/
...
static int synaptics_enable_black_gesture(struct chip_data_s3706* chip_info, bool enable) {
    ...
    if (enable) {
        ...
        if (!chip_info->report_120hz_support) {
            ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F01_RMI_CTRL02,
                                       0x3); /*set doze interval to 30ms*/
        }
    } else {
        report_gesture_ctrl_buf[2] &= 0xfd;
        if (!chip_info->report_120hz_support) {
            ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F01_RMI_CTRL02,
                                       0x1); /*set doze interval to 10ms*/
        }
    }
...
static int synaptics_corner_limit_handle(struct chip_data_s3706* chip_info, bool enable) {
    ...
    // enable is 0x02 means notch is on the left, 0x10 notch is on the right, 0x0 means close corner
    // mode
    if ((LANDSCAPE_SCREEN_90 == chip_info->touch_direction) && !enable) {
        ...
        touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL04_05,
                             0xFF);  // x part
        touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL04_06,
                             0x44);  // y part
...
static int synaptics_enable_face_detect(struct chip_data_s3706* chip_info, bool enable) {
    ...
    if (enable) {
        ret |= touch_i2c_write_byte(chip_info->client,
                                    chip_info->reg_info.F51_CUSTOM_DATA_BASE + 0x29, 0x01);
    } else {
        ret |= touch_i2c_write_byte(chip_info->client,
                                    chip_info->reg_info.F51_CUSTOM_DATA_BASE + 0x29, 0x00);
    }
    touch_i2c_write_byte(chip_info->client, 0xff, 0x00);  // set page 0
...
static void synaptics_enable_game_mode(struct chip_data_s3706* chip_info, bool enable) {
    ...
    if (chip_info->report_120hz_support) {
        ...
        ret = touch_i2c_read_byte(
                chip_info->client,
                chip_info->reg_info
                        .F51_CUSTOM_CTRL30);  // read the strong lock point level related register
        if (ret < 0) {
        ...
        ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL30,
                                   tmp);  // set the strong lock point level
        if (ret < 0) {
    ...
    ret = touch_i2c_write_block(chip_info->client, chip_info->reg_info.F12_2D_CTRL11, 13,
                                &game_buffer[0]);
    if (ret < 0) {
    ...
    TPD_INFO("%s:chip_info->reg_info.F54_ANALOG_COMMAND_BASE=0x%x\n", __func__,
             chip_info->reg_info.F54_ANALOG_COMMAND_BASE);
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                               0x04); /* force update*/
    if (ret < 0) {
...
static int synaptics_get_gesture_info(void* chip_data, struct gesture_info* gesture) {
    ...
    ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F12_2D_DATA04, 5,
                               &(gesture_buffer[0])); /*get gesture type*/
    gesture_sign = gesture_buffer[0];
    ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x4);
    ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F51_CUSTOM_DATA_BASE, 25,
                               &(coordinate_buf[0])); /*get gesture coordinate and swipe type*/
    ...
    switch (gesture_sign) {
        ...
        case UNICODE_DETECT:
            gesture->gesture_type =
                    (gesture_buffer[2] == 0x77 && gesture_buffer[3] == 0x00)   ? Wgestrue
    ...
    if (gesture->gesture_type != UnkownGesture) {
        ...
        gesture->clockwise = (coordinate_buf[24] & 0x10) ? 1
                             : (coordinate_buf[24] & 0x20)
                                     ? 0
                                     : 2; /* 1--clockwise, 0--anticlockwise, not circle, report 2*/

        TPD_INFO(
                "%s, gesture_sign: 0x%x, gesture_type: %d, clockwise: %d, points: (%d, %d)(%d, "
                "%d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n",
                __func__, gesture_sign, gesture->gesture_type, gesture->clockwise,
                gesture->Point_start.x, gesture->Point_start.y, gesture->Point_end.x,
                gesture->Point_end.y, gesture->Point_1st.x, gesture->Point_1st.y,
                gesture->Point_2nd.x, gesture->Point_2nd.y, gesture->Point_3rd.x,
                gesture->Point_3rd.y, gesture->Point_4th.x, gesture->Point_4th.y);
    }
...
unsigned char GetLogicalPin(unsigned char p_pin, uint8_t RX_NUM, uint8_t* rx_physical) {
    unsigned char i = 0;
    for (i = 0; i < RX_NUM; i++) {
        if (rx_physical[i] == p_pin) return i;
    }
    return 0xff;
}

static int synaptics_capacity_test(struct seq_file* s, struct chip_data_s3706* chip_info,
                                   struct syna_testdata* syna_testdata, struct test_header* ph,
                                   uint8_t* raw_data, uint8_t* data_buf) {
    int ret = 0;
    int x = 0, y = 0, z = 0;
    int error_count = 0;
    ...
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE,
                               0x14); /*select report type 0x14*/
    if (ret < 0) {
    ...
    ret = touch_i2c_write_byte(chip_info->client,
                               chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x14,
                               0x01); /*No SignalClarity*/
    ret = touch_i2c_read_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_CONTROL_BASE +
                                                         0x17); /*0125  CBC Xmtr carrier select*/
    tmp_arg1 = ret & 0xff;

    /*Close CBC*/
    TPD_DEBUG("ret = %x, tmp_arg1 = %x, tmp_arg2 = %x\n", ret, tmp_arg1, (tmp_arg1 & 0xdf));
    ret = touch_i2c_write_byte(chip_info->client,
                               chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x17,
                               (tmp_arg1 & 0xdf)); /*Set CBC, F54_ANALOG_CTRL88, close CBC*/
    ret = touch_i2c_write_word(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                               0x04); /*force update*/
    checkCMD(chip_info, 30);
    ...
    ret = touch_i2c_write_word(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 1,
                               0x00); /*set fifo 00*/
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                               0x01); /*get report*/
    checkCMD(chip_info, 30);
    ...
    ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3,
                               syna_testdata->TX_NUM * syna_testdata->RX_NUM * 2,
                               raw_data); /*read data*/
    for (x = 0; x < syna_testdata->TX_NUM; x++) {
        ...
        for (y = 0; y < syna_testdata->RX_NUM; y++) {
            ...
            if ((y < syna_testdata->RX_NUM - syna_testdata->key_RX) &&
                (x < syna_testdata->TX_NUM - syna_testdata->key_TX)) {
                TPD_DEBUG_NTAG("%d, ", baseline_data);
                if ((baseline_data < *(baseline_data_test + count * 2)) ||
                    (baseline_data > *(baseline_data_test + count * 2 + 1))) {
                    TPD_INFO(
                            "Synaptic:touchpanel failed, raw data erro baseline_data[%d][%d] = "
                            "%d[%d, %d]\n",
                            x, y, baseline_data, *(baseline_data_test + count * 2),
                            *(baseline_data_test + count * 2 + 1));
                    seq_printf(
                            s,
                            "touchpanel failed, raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
                            x, y, baseline_data, *(baseline_data_test + count * 2),
                            *(baseline_data_test + count * 2 + 1));
                    error_count++;
                    return error_count;
                }
            } else if ((x == syna_testdata->TX_NUM - syna_testdata->key_TX) &&
                       (y >= syna_testdata->RX_NUM - syna_testdata->key_RX)) {
                TPD_DEBUG("touchkey test xy(%d, %d), baseline_data=%d, limite( %d, %d)\n ", x, y,
                          baseline_data, *(baseline_data_test + count * 2),
                          *(baseline_data_test + count * 2 + 1));
                if (((baseline_data) < *(baseline_data_test + count * 2)) ||
                    ((baseline_data) > *(baseline_data_test + count * 2 + 1))) {
                    TPD_INFO(
                            "Synaptic:touchkey failed, raw data erro baseline_data[%d][%d] = "
                            "%d[%d, %d]\n",
                            x, y, baseline_data, *(baseline_data_test + count * 2),
                            *(baseline_data_test + count * 2 + 1));
                    seq_printf(
                            s,
                            "touchkey failed, raw data erro baseline_data[%d][%d] = %d[%d, %d]\n",
                            x, y, baseline_data, *(baseline_data_test + count * 2),
                            *(baseline_data_test + count * 2 + 1));
                    error_count++;
                    return error_count;
    ...
    ret = touch_i2c_write_byte(chip_info->client,
                               chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x14,
                               0x00); /* Disable No SignalClarity*/
    ret = touch_i2c_write_byte(chip_info->client,
                               chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x17,
                               (tmp_arg1 | 0x10)); /*open CBC*/
    ret = touch_i2c_write_word(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                               0x04);
    checkCMD(chip_info, 30);
...
static int synaptics_auto_test_rt25(struct seq_file* s, struct chip_data_s3706* chip_info,
    ...
    /*msleep(100);*/
    checkCMD(chip_info, 30);
    ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3, 7,
                               buffer);
    ...
    touch_i2c_read_block(chip_info->client, chip_info->reg_info.F55_SENSOR_CTRL02,
                         syna_testdata->TX_NUM, buffer_tx);
                ...
                if ((buffer[x] & (1 << i)) != 0) {
                    ...
                    for (j = 0; j < syna_testdata->RX_NUM; j++) {
                        if (sum_line_error == buffer_rx[j]) {
                            error_count++;
                            TPD_INFO(
                                    " step 3 :check TRx-to-Ground, with rt25 error,  error_line is "
                                    "rx = %d\n",
                                    buffer_rx[j]);
                            seq_printf(s,
                                       " step 3 :check TRx-to-Ground, with rt25,  error_line is rx "
                                       "= %d\n",
                                       buffer_rx[j]);
                            goto OUT;
                        }
                    ...
                    for (j = 0; j < syna_testdata->TX_NUM; j++) {
                        if (sum_line_error == buffer_tx[j]) {
                            error_count++;
                            TPD_INFO(
                                    " step 3 :check TRx-to-Ground, with rt25, error_line is tx = "
                                    "%d\n",
                                    buffer_tx[j]);
                            seq_printf(s,
                                       " step 3 :check TRx-to-Ground, with rt25, error_line is rx "
...
static int synaptics_auto_test_rt26(struct seq_file* s, struct chip_data_s3706* chip_info,
                                    struct syna_testdata* syna_testdata) {
    int ret = 0;
    int error_count = 0;
    int i = 0, x = 0;
    ...
    for (x = 0; x < 7; x++) {
        ...
        if (buffer[x] != 0) {
            if ((x == 0) &&
                (buffer[x] & 0x01)) {  // pin - 0, these four pins should be test in RT100
...
static int synaptics_auto_test_rt100(struct seq_file* s, struct chip_data_s3706* chip_info,
                                     struct syna_testdata* syna_testdata, uint8_t* raw_data) {
    int ret = 0;
    int error_count = 0;
    ...
    p_data_baseline1 = kzalloc(
            syna_testdata->TX_NUM * syna_testdata->RX_NUM * 2 * (sizeof(uint8_t)), GFP_KERNEL);
    if (!p_data_baseline1) {
        error_count++;
    ...
    p_data_delta = kzalloc(syna_testdata->TX_NUM * syna_testdata->RX_NUM * 2 * (sizeof(uint8_t)),
                           GFP_KERNEL);
    if (!p_data_delta) {
        error_count++;
    ...
    ret = touch_i2c_read_byte(chip_info->client,
                              chip_info->reg_info.F54_ANALOG_CONTROL_BASE);  // read no scan
    ret |= 0x02;
    touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_CONTROL_BASE,
                         ret);  // set no scan

    // set all local cbc to 0
    touch_i2c_write_block(chip_info->client, chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x1D,
                          syna_testdata->RX_NUM, &temp_data[0]);
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                               0x04); /* force update*/
    checkCMD(chip_info, 30);

    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE,
                               0x64); /*select report type 100*/
    ret = touch_i2c_write_word(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 1,
                               0x0);
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                               0x01); /*get report*/
    ...
    for (i = 0; i < 4; i++) {
        ...
        ret = touch_i2c_write_block(chip_info->client,
                                    chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x1D,
                                    syna_testdata->RX_NUM, &temp_data[0]);
        if (ret < 0) {
            error_count++;
        ...
        for (x = 0; x < syna_testdata->TX_NUM; x++) {
            for (y = 0; y < syna_testdata->RX_NUM; y++) {
                p_data_delta[x * syna_testdata->RX_NUM + y] =
                        ABS(p_data_baseline1[x * syna_testdata->RX_NUM + y],
                            p_data_baseline2[x * syna_testdata->RX_NUM + y]);
        ...
        // 18. Check data: TREX w/o CBC raised changes >= 200 or TREXn* (TREX with CBC raised)
        // changes < 2000
        ...
        for (x = 0; x < syna_testdata->RX_NUM; x++) {
            if (x == logical_pin) {
                if (minRX[x] < 600) {
                    error_count++;
                    TPD_INFO(
                            "step 5:check RT100 for pin 0,1,32,33 test failed: minRX[%-2d] = %4d "
                            "when test pin %d (RX Logical pin [%d])\n",
                            x, minRX[x], ExtendRT26_pin[i], logical_pin);
                    seq_printf(s,
                               "step 5:check RT100 for pin 0,1,32,33 test failed: minRX[%-2d] = "
                               "%4d when test pin %d (RX Logical pin [%d])\n",
                               x, minRX[x], ExtendRT26_pin[i], logical_pin);
                    goto END;
                }
            } else {
...
static int synaptics_auto_test_rt133(struct seq_file* s, struct chip_data_s3706* chip_info,
                                     struct syna_testdata* syna_testdata) {
    int ret = 0;
    int y = 0;
    int error_count = 0;
...
static int synaptics_auto_test_rt150(struct seq_file* s, struct chip_data_s3706* chip_info,
                                     struct syna_testdata* syna_testdata, uint8_t* data_buf) {
    int ret = 0;
    int y = 0;
    int error_count = 0;
    uint8_t tmp_arg1 = 0, tmp_arg2 = 0;
    ...
    ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01); /* page 1*/
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE,
                               0x96); /*select report type RT150*/
    ret = touch_i2c_write_word(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 1,
                               0x00); /*set fifo 00*/
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                               0x01); /*get report*/
    checkCMD(chip_info, 70);
    for (y = 0; y < syna_testdata->RX_NUM + syna_testdata->TX_NUM - syna_testdata->key_TX -
                            syna_testdata->key_RX;
         y++) {
        ret = touch_i2c_read_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3);
        tmp_arg1 = ret & 0xff;
        ret = touch_i2c_read_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3);
        ...
        if (unsigned_baseline_data < 7000) {
            error_count++;
            TPD_INFO(
                    "Step 7 : Check RT150 for random touch event, error_line is y =%d, data[%d] = "
                    "%hu\n",
...
static int synaptics_auto_test_rt154(struct seq_file* s, struct chip_data_s3706* chip_info,
                                     struct syna_testdata* syna_testdata, uint8_t* data_buf) {
    int ret = 0;
    int error_count = 0;
    int y = 0;
    ...
    for (y = 0; y < chip_info->hw_res->RX_NUM + chip_info->hw_res->TX_NUM; y++) {
        ...
        if ((syna_testdata->key_RX != 0 || syna_testdata->key_TX != 0) &&
            (((y > chip_info->hw_res->RX_NUM - syna_testdata->key_RX - 1) &&
              (y < chip_info->hw_res->RX_NUM)) ||
             (y >
              chip_info->hw_res->RX_NUM + chip_info->hw_res->TX_NUM - syna_testdata->key_TX - 1))) {
            TPD_INFO("y = %d\n", y);
            continue;
        }
        if (unsigned_baseline_data < 50 || unsigned_baseline_data > 975) {
            error_count++;
            TPD_INFO(
                    "Step 8 : Check RT154 for self raw data check, error_line is y =%d, data[%d] = "
                    "%hu\n",
                    y, y, unsigned_baseline_data);
            seq_printf(s,
                       "Step 8 : Check RT154 for self raw data check, error_line is y =%d, "
                       "data[%d] = %hu\n",
                       y, y, unsigned_baseline_data);
            return error_count;
        }
...
static int synaptics_auto_test_rt155(struct seq_file* s, struct chip_data_s3706* chip_info,
                                     struct syna_testdata* syna_testdata, uint8_t* data_buf) {
    int ret = 0;
    int error_count = 0;
    int y = 0;
    ...
    ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01); /* page 1*/
    ret = touch_i2c_read_byte(chip_info->client,
                              chip_info->reg_info.F54_ANALOG_CONTROL_BASE); /*get 0x10E status*/
    ret |= 0x01;
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_CONTROL_BASE,
                               ret); /*set no relax*/
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                               0x04); /* force update*/
    checkCMD(chip_info, 30);
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE,
                               0x9B); /*select report type 0x9B*/
    ret = touch_i2c_write_word(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 1,
                               0x00); /*set fifo 00*/
    ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                               0x01); /*get report*/
    ...
    for (y = 0; y < chip_info->hw_res->RX_NUM + chip_info->hw_res->TX_NUM; y++) {
        ...
        if (baseline_data > 2000) {
            error_count++;
            TPD_INFO(
                    "Step 9 : Check RT155 for abs delta error, error_line is y =%d, data[%d] = "
                    "%d\n",
                    y, y, baseline_data);
            seq_printf(s,
                       "Step 9 : Check RT155 for abs delta error, error_line is y =%d, data[%d] = "
...
static int synaptics_auto_test_rt155_fd(struct seq_file* s, struct chip_data_s3706* chip_info,
                                        struct syna_testdata* syna_testdata, uint8_t* data_buf) {
...
static int synaptics_auto_test_rt59(struct seq_file* s, struct chip_data_s3706* chip_info,
                                    struct syna_testdata* syna_testdata, uint8_t* data_buf) {
    int ret = 0;
    int error_count = 0;
    int i = 0, j = 0;
    const int data_cycle = 20, node_num = syna_testdata->TX_NUM + syna_testdata->RX_NUM,
              judge_threshold = 300;
    ...
    for (i = 0; i < data_cycle; i++) {
        ...
        checkCMD(chip_info, 30);
        ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3,
                                   node_num * 4, raw_data); /*read data*/
        for (j = 0; j < node_num; j++) {
            node_data = (raw_data[j * 4] | (raw_data[j * 4 + 1] << 8) |
                         (raw_data[j * 4 + 2] << 16) | (raw_data[j * 4 + 3] << 24));
...
static int synaptics_auto_test_rt63(struct seq_file* s, struct chip_data_s3706* chip_info,
                                    struct syna_testdata* syna_testdata, uint8_t* data_buf) {
    ...
    for (i = 0; i < syna_testdata->RX_NUM; i++) {
        node_data = (raw_data[i * 4] | (raw_data[i * 4 + 1] << 8) | (raw_data[i * 4 + 2] << 16) |
                     (raw_data[i * 4 + 3] << 24));
        if (syna_testdata->fd >= 0) {
        ...
        if ((node_data < 10000) || (node_data > 60000)) {
            ...
            if (!error_count) seq_printf(s, "Check RT63 for hover, data[%d] = %d\n", i, node_data);
            error_count++;
        }
    }
    if (syna_testdata->fd >= 0) {
...
static void synaptics_auto_test(struct seq_file* s, void* chip_data,
                                struct syna_testdata* syna_testdata) {
    int ret = 0;
    int error_count = 0;
    ...
    struct monitor_data_v2* mon_data_v2 = chip_info->monitor_data_v2;

    if (!mon_data_v2) {
        TPD_INFO("%s: mon_data_v2 is NULL\n", __func__);
    }

    raw_data = kzalloc(syna_testdata->TX_NUM * syna_testdata->RX_NUM * 2 * (sizeof(uint8_t)),
                       GFP_KERNEL);
    if (!raw_data) {
    ...
    if (eint_count == 10) {
        tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "TP EINT PIN direct short");
        error_count++;
    ...
    if (ret > 0) {
        ...
        tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "synaptics_capacity_test failed");
        error_count++;
        goto END;
    }
    ...
    if (ret > 0) {
        ...
        tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "synaptics_auto_test_rt25 failed");
        error_count++;
        goto END;
    }
    synaptics_reset(chip_info);
    ...
    if (ret > 0) {
        ...
        tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "synaptics_auto_test_rt26 failed");
        error_count++;
        goto END;
    }
    synaptics_reset(chip_info);
    ...
    if (ret > 0) {
        ...
        tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "synaptics_auto_test_rt100 failed");
        error_count++;
        goto END;
    }
    synaptics_reset(chip_info);
    ...
    if (ret > 0) {
        ...
        tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "synaptics_auto_test_rt133 failed");
        error_count++;
        goto END;
    }
    ...
    if (ret > 0) {
        ...
        tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "synaptics_auto_test_rt150 failed");
        error_count++;
        goto END;
    }
    ...
    if (ret > 0) {
        ...
        tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "synaptics_auto_test_rt154 failed");
        error_count++;
        goto END;
    }
    ...
    if (!syna_testdata->fingerprint_underscreen_support) {
        ...
        if (ret > 0) {
            ...
            tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "synaptics_auto_test_rt155 failed");
            error_count++;
            goto END;
        }
    }
    ...
    if (syna_testdata->fd_support) {
        ...
        if (ret > 0) {
            ...
            tp_healthinfo_report(mon_data_v2, HEALTH_REPORT,
                                 "synaptics_auto_test_rt59/rt155 failed");
            error_count++;
            goto END;
        }
        ...
        if (ret > 0) {
            ...
            tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "synaptics_auto_test_rt63 failed");
            error_count++;
            goto END;
        }
        synaptics_reset(chip_info);
...
END:
    tp_healthinfo_report(mon_data_v2, HEALTH_TEST_AUTO, &error_count);
    kfree(raw_data);
...
static void synaptics_baseline_read(struct seq_file* s, void* chip_data) {
    ...
    raw_data =
            kzalloc(chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM * 2 * (sizeof(uint8_t)),
                    GFP_KERNEL);
    if (!raw_data) {
        TPD_INFO("raw_data kzalloc error\n");
        return;
    ...
    do {
        ...
        if (enable_cbc) {
            ...
            ret = touch_i2c_write_word(chip_info->client,
                                       chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x04);
            checkCMD(chip_info, 30);
            TPD_DEBUG("Test open cbc\n");
        } else {
            seq_printf(s, "\nWithout CBC:\n");
            ...
            ret = touch_i2c_write_byte(chip_info->client,
                                       chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 20,
                                       0x01); /* Enable No SignalClarity*/
            ret = touch_i2c_write_byte(chip_info->client,
                                       chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 0x17,
                                       (tmp_arg1 & 0xef));
            ret = touch_i2c_write_word(chip_info->client,
                                       ...
                                       0x04); /* force update*/
            ret = touch_i2c_write_byte(chip_info->client,
                                       chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 7,
                                       0x01); /* Forbid NoiseMitigation*/
        }
        /******write No Relax to 1******/
        ret = touch_i2c_write_word(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                                   0x04); /* force update*/
...
static void synaptics_RT251_read(struct seq_file* s, void* chip_data) {
    ...
    checkCMD(chip_info, 70);
    seq_printf(s, "\n[RT252]");
    for (y = 0; y < chip_info->hw_res->RX_NUM + chip_info->hw_res->TX_NUM -
                            chip_info->hw_res->key_TX - chip_info->hw_res->key_RX;
         y++) {
        ret = touch_i2c_read_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3);
        tmp_arg1 = ret & 0xff;
        ret = touch_i2c_read_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3);
...
static void synaptics_get_gesture_coord(void* chip_data, uint32_t gesture_type) {
    ...
    if (gesture_type == DouSwip) {
        ...
        memset(str_gesture, 0, GESTURE_COORD_LEN);
        memset(raw_data, 0,
               chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM * 2 * (sizeof(uint8_t)));
        /*TPD_DEBUG("\nstep 2:report type3 delta image\n");*/
        ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE,
                                   0x03); /*select report type 0x02*/
        ret = touch_i2c_write_word(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 1,
                                   0x00); /*set fifo 00*/
        ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                                   0x01); /*get report*/
...
static int fwu_recovery_check_status(struct chip_data_s3706* chip_info) {
    ...
    retval = touch_i2c_read_block(chip_info->client, data_base + F35_ERROR_CODE_OFFSET, 1, &status);
    if (retval < 0) {
...
static int fwu_scan_pdt(struct chip_data_s3706* chip_info) {
    ...
    for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
        retval = touch_i2c_read_block(chip_info->client, addr, sizeof(rmi_fd),
        ...
        if (rmi_fd.fn_number) {
            ...
            switch (rmi_fd.fn_number) {
                ...
                case SYNAPTICS_RMI4_F34: /*found*/
                    f34found = true;
                    chip_info->fwu->f34_fd.query_base_addr = rmi_fd.query_base_addr;
                    chip_info->fwu->f34_fd.ctrl_base_addr = rmi_fd.ctrl_base_addr;
                    chip_info->fwu->f34_fd.data_base_addr = rmi_fd.data_base_addr;
    ...
    rmi4_data->intr_mask[0] |=
            chip_info->fwu->intr_mask;  // Roland interrupts mode for write flash, like
                                        // checkFlashState in 3508 notice

    addr = rmi4_data->f01_ctrl_base_addr + 1;

    retval = touch_i2c_write_block(chip_info->client, addr, sizeof(rmi4_data->intr_mask[0]),
                                   &(rmi4_data->intr_mask[0]));
    if (retval < 0) {
...
static int fwu_write_f34_v7_partition_id(struct chip_data_s3706* chip_info, unsigned char cmd) {
    ...
    retval = touch_i2c_write_block(chip_info->client, data_base + chip_info->fwu->off.partition_id,
...
static int fwu_read_flash_status(struct chip_data_s3706* chip_info) {
    ...
    if (chip_info->fwu->flash_status != 0x00) {
        TPD_INFO("%s: Flash status = %d, command = 0x%02x\n", __func__,
                 chip_info->fwu->flash_status, chip_info->fwu->command);
    }
    ...
    retval = touch_i2c_read_block(
            chip_info->client,
            chip_info->fwu->f34_fd.data_base_addr + chip_info->fwu->off.flash_cmd, sizeof(command),
...
static int fwu_write_f34_v7_command_single_transaction(struct chip_data_s3706* chip_info,
                                                       unsigned char cmd) {
    int retval;
    unsigned char data_base;
...
static int fwu_write_f34_v7_command(struct chip_data_s3706* chip_info, unsigned char cmd) {
    ...
    retval = touch_i2c_write_block(chip_info->client, data_base + chip_info->fwu->off.flash_cmd,
...
static int fwu_read_f34_v7_partition_table(struct chip_data_s3706* chip_info,
    ...
    retval = touch_i2c_write_block(chip_info->client, data_base + chip_info->fwu->off.block_number,
    ...
    retval = touch_i2c_write_block(chip_info->client,
                                   data_base + chip_info->fwu->off.transfer_length, sizeof(length),
                                   length);
    if (retval < 0) {
    ...
    retval = touch_i2c_read_block(chip_info->client, data_base + chip_info->fwu->off.payload,
                                  chip_info->fwu->partition_table_bytes, partition_table);
    if (retval < 0) {
...
static void fwu_parse_partition_table(struct chip_data_s3706* chip_info,
    ...
    for (ii = 0; ii < chip_info->fwu->partitions; ii++) {
        ...
        physical_address =
                ptable->start_physical_address_15_8 << 8 | ptable->start_physical_address_7_0;
...
static int fwu_read_f34_v7_queries(struct chip_data_s3706* chip_info) {
    ...
    retval =
            touch_i2c_read_block(chip_info->client, query_base, sizeof(query_0.data), query_0.data);
    if (retval < 0) {
    ...
    retval = touch_i2c_read_block(chip_info->client, query_base + offset, sizeof(query_1_7.data),
                                  query_1_7.data);
    if (retval < 0) {
    ...
    chip_info->fwu->block_size = query_1_7.block_size_15_8 << 8 | query_1_7.block_size_7_0;

    chip_info->fwu->flash_config_length =
            query_1_7.flash_config_length_15_8 << 8 | query_1_7.flash_config_length_7_0;

    chip_info->fwu->payload_length =
            query_1_7.payload_length_15_8 << 8 | query_1_7.payload_length_7_0;
    ...
    fwu_parse_partition_table(chip_info, ptable, &chip_info->fwu->blkcount,
                              &chip_info->fwu->phyaddr);  // firmware in IC condition
...
static int fwu_get_device_config_id(struct chip_data_s3706* chip_info) {
    ...
    retval = touch_i2c_read_block(chip_info->client, chip_info->fwu->f34_fd.ctrl_base_addr,
                                  config_id_size, chip_info->fwu->config_id);
    if (retval < 0) {
        return retval;
    }
...
static int synaptics_rmi4_fwu_init(struct chip_data_s3706* chip_info, bool force) {
    ...
    retval = touch_i2c_read_block(chip_info->client, PDT_PROPS, sizeof(pdt_props.data),
                                  pdt_props.data);
    if (retval < 0) {
    ...
    chip_info->fwu->rmi4_data->sensor_sleep =
            0; /*don't enter sensor sleep or suspend while update*/
...
static unsigned int le_to_uint(const unsigned char* ptr) {
    return (unsigned int)ptr[0] + (unsigned int)ptr[1] * 0x100 + (unsigned int)ptr[2] * 0x10000 +
           (unsigned int)ptr[3] * 0x1000000;
}

static void fwu_parse_image_header_10_bootloader(struct chip_data_s3706* chip_info,
...
static int fwu_parse_image_info(struct chip_data_s3706* chip_info) {
    ...
    if (chip_info->fwu->bl_version == BL_V7 || chip_info->fwu->bl_version == BL_V8) {
        ...
        fwu_parse_partition_table(chip_info, chip_info->fwu->img.fl_config.data,
                                  &chip_info->fwu->img.blkcount,
                                  &chip_info->fwu->img.phyaddr);  // firmware img in emmc
        ...
        fwu_compare_partition_tables(
                chip_info);  // compare two firmware img between ic and emmc, compare addr , if not
                             // equal, return or reflash partition
    } else {
        chip_info->fwu->new_partition_table = false;
        chip_info->fwu->incompatible_partition_tables = false;
    }
...
static enum flash_area fwu_go_nogo(struct chip_data_s3706* chip_info) {
    ...
    retval = fwu_get_image_firmware_id(
            chip_info,
            &image_fw_id); /*the firmware id in the img, if not equal device firmware id , update*/
    if (retval < 0) {
        flash_area = NONE;
        goto exit;
    }
    ...
    if (image_fw_id != device_fw_id) {
        flash_area = UI_FIRMWARE; /*UI_FIRMWARE means to update firmware and config, no need to
                                     judge config*/
        goto exit;
    }
...
static int fwu_enter_flash_prog(struct chip_data_s3706* chip_info) {
    ...
    retval = touch_i2c_read_block(chip_info->client, chip_info->fwu->rmi4_data->f01_ctrl_base_addr,
                                  sizeof(f01_device_control.data), f01_device_control.data);
    if (retval < 0) {
    ...
    retval = touch_i2c_write_block(chip_info->client, chip_info->fwu->rmi4_data->f01_ctrl_base_addr,
...
static int fwu_write_f34_v7_blocks(struct chip_data_s3706* chip_info, unsigned char* block_ptr,
    ...
    do {
        ...
        do {
            ...
            retval = touch_i2c_write_block(chip_info->client,
                                           data_base + chip_info->fwu->off.payload, write_size,
                                           block_ptr);
            if (retval < 0) {
...
static int fwu_erase_all(struct chip_data_s3706* chip_info) {
    ...
    if (chip_info->fwu->bl_version == BL_V7) {
        ...
        if (chip_info->fwu->bl_version == BL_V8) return 0;
    }
...
static void calculate_checksum(unsigned short* data, unsigned long len, unsigned long* result) {
    unsigned long temp;
...
static int fwu_write_utility_parameter(struct chip_data_s3706* chip_info) {
    ...
    for (ii = 0; ii < MAX_UTILITY_PARAMS; ii++) {
        if (chip_info->fwu->img.utility_param_id[ii] == UNUSED) continue;
        ...
        if (fwu->img.utility_param_id[ii] == FORCE_PARAMETER) {
            ...
            retval = secure_memcpy(&(pbuf[4]), remaining_size - 4, fwu->cal_data,
    ...
    image_param:
        ...
        retval = secure_memcpy(pbuf, remaining_size, chip_info->fwu->img.utility_param[ii].data,
...
static int fwu_read_f34_blocks(struct chip_data_s3706* chip_info, unsigned short block_cnt,
                               unsigned char cmd) {
    int retval = 0;
...
static int fwu_do_reflash(struct chip_data_s3706* chip_info) {
    ...
    if (!chip_info->fwu->new_partition_table) {
        retval = fwu_check_ui_firmware_size(
                chip_info);  // check firmware block nums , a block is 16bytes
        if (retval < 0) return retval;
    ...
    if (do_bl_update) {
        ...
        retval = fwu_write_partition_table_v8(
                chip_info);  // partition table is earsed in fwu_erase_all, rewrite
                             // FLASH_CONFIG_AREA partition
        if (retval < 0) return retval;
        TPD_INFO("%s: Partition table programmed\n", __func__);
    }
...
static int fwu_start_reflash(struct chip_data_s3706* chip_info) {
    ...
    struct synaptics_rmi4_data* rmi4_data = chip_info->fwu->rmi4_data;
    struct monitor_data_v2* monitor_data_v2 = chip_info->monitor_data_v2;

    if (!monitor_data_v2) {
        TPD_INFO("%s: monitor_data_v2 is NULL\n", __func__);
    }

    if (rmi4_data->sensor_sleep) {
        TPD_INFO("%s: Sensor sleeping\n", __func__);
        tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE, "Sensor error sleeping");
        return -ENODEV;
    }
    ...
    if (retval < 0) {
        tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE, "Parse image info failed");
        goto exit;
    }

    if (chip_info->fwu->blkcount.total_count != chip_info->fwu->img.blkcount.total_count) {
        TPD_INFO("%s: Flash size mismatch\n", __func__);
        tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE, "Flash size error mismatch");
        retval = -EINVAL;
        goto exit;
    }
    ...
    if (chip_info->fwu->bl_version != chip_info->fwu->img.bl_version) {
        ...
        tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE,
                             "Bootloader version error mismatch");
        retval = -EINVAL;
        goto exit;
    }
    ...
    retval = fwu_read_flash_status(chip_info);
    if (retval < 0) {
        tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE, "Read flash status failed");
        goto exit;
    }
    ...
    if (flash_area != NONE) {
        retval = fwu_enter_flash_prog(chip_info);
        /*lots of info, large function*/  // enter bootloader mode
        if (retval < 0) {
            tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE, "Enter flash prog failed");
            chip_info->fwu->rmi4_data->reset_device(rmi4_data, false);
            goto exit;
        }
    }
    ...
    switch (flash_area) {
        ...
        case UI_CONFIG:
            ...
            retval = fwu_check_ui_configuration_size(chip_info);
            if (retval < 0) {
                tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE, "Check UI config failed");
                break;
            }
            chip_info->fwu->config_area =
                    UI_CONFIG_AREA; /*fwu_erase_configuration should set config_area first*/
            retval = fwu_erase_configuration(chip_info);
            if (retval < 0) {
                tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE, "Erase UI config failed");
                break;
            }
            retval = fwu_write_ui_configuration(chip_info);
            if (retval < 0) {
                tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE, "Write UI config failed");
            }
            break;
        case NONE:
        default:
    ...
    if (chip_info->fwu->do_lockdown && (chip_info->fwu->img.lockdown.data !=
                                        NULL)) {  // lockdown mode , only can write once, use for
                                                  // save i2c addr, pull up, pull down info
        ...
        switch (chip_info->fwu->bl_version) {
            ...
            case BL_V8:
                ...
                if (retval < 0) {
                    tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE, "Lockdown failed");
...
exit:
    if (retval >= 0) {
        tp_healthinfo_report(monitor_data_v2, HEALTH_FW_UPDATE, "FW Update Success");
    }
    if (do_rebuild) chip_info->fwu->rmi4_data->reset_device(rmi4_data, true);
...
static fp_touch_state synaptics_spurious_fp_check(void* chip_data) {
    ...
    touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE,
                         0x7c); /*select report type 124*/
    touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 1,
                         0x00); /*set LSB*/
    touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 2,
                         0x00); /*set MSB*/
    touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
                         0x01); /*get report*/
    ...
    touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3,
                         TX_NUM * SPURIOUS_FP_RX_NUM * 2, raw_data); /*read baseline data*/
    ...
    TPD_INFO("%s:%d chip_info->reg_info.F54_ANALOG_COMMAND_BASE=0x%x set 0, \n", __func__, __LINE__,
             chip_info->reg_info.F54_ANALOG_COMMAND_BASE); /*add for Prevent TP failure*/
...
static u8 synaptics_get_keycode(void* chip_data) {
    ...
    if (ret & 0x01) SET_BIT(bitmap_result, BIT_MENU);
    if (ret & 0x02) SET_BIT(bitmap_result, BIT_BACK);
...
RE_TRY:
    ...
    ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3,
                               TX_NUM * SPURIOUS_FP_RX_NUM * 2, raw_data); /*read data*/
    if (ret < 0) {
        if (retry_time) {
...
static void synaptics_health_report(void* chip_data, struct monitor_data* mon_data) {
    ...
    ret = touch_i2c_write_byte(chip_info->client, 0xff,
                               (uint8_t)(chip_info->health_zone.loglength_addr >> 8)); /* page 4*/
    data_length = touch_i2c_read_byte(chip_info->client,
                                      (uint8_t)(chip_info->health_zone.loglength_addr & 0xFF));
    if (data_length < 1 || data_length > 255) {
        return;
    } else {
        ret = touch_i2c_read_block(chip_info->client,
                                   ((uint8_t)(chip_info->health_zone.loglength_addr & 0xFF) + 1),
                                   data_length, log_data); /*read log*/
...
static void synaptics_health_report_v2(void* chip_data, struct monitor_data_v2* mon_data_v2) {
    int ret = 0, i = 0;
    int data_length = 0;
    uint8_t log_data[255];
    struct chip_data_s3706* chip_info = (struct chip_data_s3706*)chip_data;
    ...
    if (data_length < 1 || data_length > 255) {
        ...
        TPD_DEBUG("data_length = %d\n", data_length);
        if (health_info->grip_count != 0 && health_local->grip_count != health_info->grip_count) {
            tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, HEALTH_REPORT_GRIP);
        }
        if (health_info->baseline_err != 0 &&
            health_local->baseline_err != health_info->baseline_err) {
            switch (health_info->baseline_err) {
                case BASE_NEGATIVE_FINGER:
                    ...
                    break;
                case BASE_MUTUAL_SELF_CAP:
                    tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "base_mutual_self_cap");
                    break;
                case BASE_ENERGY_RATIO:
                    tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "base_energy_ratio");
                    break;
                case BASE_RXABS_BASELINE:
                    tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "base_rxabs_baseline");
                    break;
                case BASE_TXABS_BASELINE:
                    tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, "base_txabs_baseline");
                    break;
                default:
                    break;
            }
        ...
        if (health_info->shield_mode != 0 &&
            ...
            switch (health_info->shield_mode) {
                case SHIELD_PALM:
                    ...
                    break;
                case SHIELD_GRIP:
                    tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, HEALTH_REPORT_SHIELD_EDGE);
                    break;
                case SHIELD_METAL:
                    tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, HEALTH_REPORT_SHIELD_METAL);
                    break;
                case SHIELD_MOISTURE:
                    tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, HEALTH_REPORT_SHIELD_WATER);
                    break;
                case SHIELD_ESD:
                    tp_healthinfo_report(mon_data_v2, HEALTH_REPORT, HEALTH_REPORT_SHIELD_ESD);
                    break;
                default:
                    break;
            }
...
static void synaptics_gesture_rate(struct seq_file* s, u16* coord_arg, void* chip_data) {
    ...
    if (coord_arg[0] == DouTap) {
        ...
        ret = touch_i2c_write_word(chip_info->client, 0x25,
                                   coord_arg[1] + coord_arg[105]); /*write length*/
    ...
    for (loop = 0; loop < 2 * coord_arg[1]; loop++) {
        ret = touch_i2c_write_byte(chip_info->client, 0x27,
                                   *(coord + loop + 6)); /*write coordinate*/
    }
    if (coord_arg[0] == DouSwip) {
        for (loop = 0; loop < 2 * coord_arg[105]; loop++) {
            ret = touch_i2c_write_byte(chip_info->client, 0x27,
                                       *(coord + loop + 4 + 210)); /*write coordinate*/
        }
    }
    TPD_DETAIL("coord[0]:%u %u %u %u %u\n", coord_arg[0], coord_arg[1], coord_arg[3], coord_arg[4],
               coord_arg[5]);
    ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x00); /* page 4*/
}

static void synaptics_register_info_read(void* chip_data, uint16_t register_addr, uint8_t* result,
                                         uint8_t length) {
    int ret = 0;
    uint8_t l_tmp = 0, h_tmp = 0;
    struct chip_data_s3706* chip_info = (struct chip_data_s3706*)chip_data;
...
static int synaptics_get_face_state(void* chip_data) {
    ...
    if (ret >= 0) TPD_INFO("ps value : 0x%02x\n", state);

    // reset for rotation, screen color black to white, regard as near
    if ((ret > 0) && time_before(jiffies, chip_info->rotation_changed_time + HZ) &&
        chip_info->rotation_changed_time) {
...
static void synaptics_screenon_fingerprint_info(void* chip_data,
...
static int synaptics_set_report_point_first(void* chip_data, uint32_t enable) {
    ...
    if (enable) {  // enable game mode control flag
        ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL13,
                                   (ret | 0x04));
        if (ret < 0) {
            touch_i2c_write_byte(chip_info->client, 0xff, 0x00); /* page 0*/
...
static struct oppo_touchpanel_operations synaptics_ops = {
        ...
        .health_report_v2 = synaptics_health_report_v2,
#if GESTURE_COORD_GET
...
static int synaptics_tp_probe(struct i2c_client* client, const struct i2c_device_id* id) {
    ...
    chip_info->fw_corner_limit_support =
            of_property_read_bool(ts->dev->of_node, "fw_corner_limit_support");
    chip_info->rt155_fdreplace_rt59_support =
            of_property_read_bool(ts->dev->of_node, "rt155_fdreplace_rt59_support");
    chip_info->report_120hz_support =
            of_property_read_bool(ts->dev->of_node, "report_120hz_support");
...
static const struct i2c_device_id tp_id[] = {{TPD_DEVICE, 0}, {}};
...
static int __init tp_driver_init(void) {
    ...
    if (!tp_judge_ic_match(TPD_DEVICE)) return -1;
#endif
