...
struct panel_info {
    ...
    u32 project_id;
    uint32_t platform_support_project[12];
    uint32_t platform_support_project_dir[12];
    char* platform_support_commandline[12];
    char* platform_support_external_name[12];
    int project_num;
...
struct freq_hop_info {
    ...
    int freq_hop_freq; /*save frequency-hopping frequency, trigger frequency-hopping every
...
struct touchpanel_data {
    ...
    bool auto_test_need_cal_support; /*multiple panel consistency if contains samsung ic to
                                        calivation*/
    ...
    u8 vk_bitmap;    /*every bit declear one state of key
    ...
    int firmware_update_type; /*firmware_update_type: 0=check firmware version 1=force update; 2=for
                                 FAE debug*/
    ...
    bool report_point_first_support;   /*if it happened baseline error,reporting point first or
                                          reporting error first. */
...
struct oppo_touchpanel_operations {
    ...
    int (*power_control)(void* chip_data,
    ...
    u8 (*trigger_reason)(void* chip_data, int gesture_enable,
    ...
    void (*resume_prepare)(void* chip_data); /*using for operation before resume flow,
                                               eg:incell 3320 need to disable gesture to release
                                               inter pins for lcd resume*/
    ...
    void (*register_info_read)(void* chip_data, uint16_t register_addr, uint8_t* result,
                               uint8_t length); /*add for read registers*/
    void (*write_ps_status)(
            void* chip_data,
            int ps_status); /*when detect iron plate, if ps is near ,enter iron plate mode;if ps is
                               far, can not enter; exit esd mode when ps is far*/
    ...
    void (*health_report_v2)(void* chip_data,
                             struct monitor_data_v2* mon_data_v2); /*data logger get*/
    void (*bootup_test)(void* chip_data, const struct firmware* fw, struct monitor_data* mon_data,
