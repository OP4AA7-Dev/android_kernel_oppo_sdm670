...
#define TPD_DEBUG(a, arg...)                                                  \
    do {                                                                      \
        if (LEVEL_DEBUG == tp_debug) pr_err("[TP]" TPD_DEVICE ": " a, ##arg); \
    } while (0)

#define TPD_DETAIL(a, arg...)                                                 \
    do {                                                                      \
        if (LEVEL_BASIC != tp_debug) pr_err("[TP]" TPD_DEVICE ": " a, ##arg); \
    } while (0)
...
__attribute__((weak)) int notify_prevention_handle(struct kernel_grip_info* grip_info,
...
void operate_mode_switch(struct touchpanel_data* ts) {
    ...
    if (ts->is_suspended) {
        ...
        if (ts->smooth_level_array_support && ts->ts_ops->smooth_lv_set) {
            if (ts->smooth_level_charging_array_support && ts->charger_pump_support &&
                ts->is_usb_checked) {
...
static void tp_touch_down(struct touchpanel_data* ts, struct point_info points,
    ...
    if (ts->boot_mode == MSM_BOOT_MODE__RECOVERY) {
        ...
        if (touch_report_num == 1) {
            ...
            // report move event after down event, for weixing voice delay problem, 30 -> 300ms in
            // order to avoid the intercept by shortcut
        ...
        if ((points.x > ts->touch_major_limit.width_range) &&
            ...
            (points.y < ts->resolution_info.max_y - ts->touch_major_limit.height_range)) {
...
static void tp_geture_info_transform(struct gesture_info* gesture,
    ...
    gesture->Point_start.x =
            gesture->Point_start.x * resolution_info->LCD_WIDTH / (resolution_info->max_x);
    gesture->Point_start.y =
            gesture->Point_start.y * resolution_info->LCD_HEIGHT / (resolution_info->max_y);
    gesture->Point_end.x =
            gesture->Point_end.x * resolution_info->LCD_WIDTH / (resolution_info->max_x);
    gesture->Point_end.y =
            gesture->Point_end.y * resolution_info->LCD_HEIGHT / (resolution_info->max_y);
    gesture->Point_1st.x =
            gesture->Point_1st.x * resolution_info->LCD_WIDTH / (resolution_info->max_x);
    gesture->Point_1st.y =
            gesture->Point_1st.y * resolution_info->LCD_HEIGHT / (resolution_info->max_y);
    gesture->Point_2nd.x =
            gesture->Point_2nd.x * resolution_info->LCD_WIDTH / (resolution_info->max_x);
    gesture->Point_2nd.y =
            gesture->Point_2nd.y * resolution_info->LCD_HEIGHT / (resolution_info->max_y);
    gesture->Point_3rd.x =
            gesture->Point_3rd.x * resolution_info->LCD_WIDTH / (resolution_info->max_x);
    gesture->Point_3rd.y =
            gesture->Point_3rd.y * resolution_info->LCD_HEIGHT / (resolution_info->max_y);
    gesture->Point_4th.x =
            gesture->Point_4th.x * resolution_info->LCD_WIDTH / (resolution_info->max_x);
    gesture->Point_4th.y =
...
static bool edge_point_process(struct touchpanel_data* ts, struct point_info points) {
    if (ts->limit_edge) {
        if (points.x > ts->edge_limit.left_x2 && points.x < ts->edge_limit.right_x2) {
            if (ts->edge_limit.in_which_area == AREA_EDGE) tp_touch_release(ts);
            ts->edge_limit.in_which_area = AREA_NORMAL;
        } else if ((points.x > ts->edge_limit.left_x1 && points.x < ts->edge_limit.left_x2) ||
            ...
            if (ts->edge_limit.in_which_area == AREA_NORMAL) return true;
...
static bool corner_point_process(struct touchpanel_data* ts, struct corner_info* corner,
...
static int touch_elimination_caculate(struct touchpanel_data* ts, int obj_attention,
    ...
    for (i = 0; i < ts->max_num; i++) {
        if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01) {
            ...
            if (ts->limit_edge) {
                ...
                } else if ((points[i].x > ts->monitor_data.eli_ver_range) &&
                           (points[i].x <
                            ts->resolution_info.max_x - ts->monitor_data.eli_ver_range)) {
                ...
                if (points[i].y <= ts->monitor_data.eli_hor_range) {
                    left_edge_cnt++;
                    left_edge_bit = left_edge_bit | (1 << i);
                } else if ((points[i].y > ts->monitor_data.eli_hor_range) &&
                           (points[i].y < ts->resolution_info.max_y / 2)) {
                    cross_status[i] = 1;  // set cross status of i
                    left_center_cnt++;
                } else if ((points[i].y >= ts->resolution_info.max_y / 2) &&
                           (points[i].y <
                            ts->resolution_info.max_y - ts->monitor_data.eli_hor_range)) {
                    cross_status[i] = 1;  // set cross status of i
                    right_center_cnt++;
                } else {
                    right_edge_cnt++;
            ...
            for (i = 0; i < ts->max_num; i++) {
                if ((((left_edge_bit & TOUCH_BIT_CHECK) >> i) & 0x01) && !cross_status[i]) {
                    ...
                    if (!left_trigger_status && ts->monitor_data.eli_ver_pos) {
                        if (points[i].x <= ts->monitor_data.eli_ver_range)
                            ts->monitor_data.eli_ver_pos[points[i].x / unit * max_array_y +
                                                         points[i].y / unit]++;
                        else
                            ts->monitor_data.eli_ver_pos[(ts->resolution_info.max_x - points[i].x) /
                                                                 unit * max_array_y +
                                                         points[i].y / unit]++;
                    }
                }
            ...
            for (i = 0; i < ts->max_num; i++) {
                if ((((left_edge_bit & TOUCH_BIT_CHECK) >> i) & 0x01) && !cross_status[i]) {
                    ...
                    if (!left_trigger_status && ts->monitor_data.eli_hor_pos) {
                        ts->monitor_data
                                .eli_hor_pos[points[i].y / unit * max_array_x +
                                             (ts->resolution_info.max_x - points[i].x) / unit]++;
                    }
                }
            }
            ...
            for (i = 0; i < ts->max_num; i++) {
                if ((((right_edge_bit & TOUCH_BIT_CHECK) >> i) & 0x01) && !cross_status[i]) {
                    ...
                    if (!right_trigger_status && ts->monitor_data.eli_hor_pos) {
                        ts->monitor_data
                                .eli_hor_pos[(ts->resolution_info.max_y - points[i].y) / unit *
                                                     max_array_x +
                                             (ts->resolution_info.max_x - points[i].x) / unit]++;
                    }
                }
        ...
        for (i = 0; i < ts->max_num; i++) {
            ...
            if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01 && (points[i].status != 0)) {
                ...
                if (ts->edge_limit_support) {
                    if (corner_point_process(ts, corner, points, i) ||
                        (!ts->drlimit_remove_support && edge_point_process(ts, points[i])))
                        continue;
                }
#ifdef TYPE_B_PROTOCOL
                input_mt_slot(ts->input_dev, i);
                ...
                if (ts->ear_sense_support && ts->es_enable &&
                    (((points[i].y < ts->resolution_info.max_y / 2) && (points[i].x > 30) &&
                     ...
                     ((points[i].y > ts->resolution_info.max_y / 2) && (points[i].x > 70) &&
                      (points[i].x < ts->resolution_info.max_x - 70)))) {
                    finger_num_center++;
                }
...
static void tp_btnkey_release(struct touchpanel_data* ts) {
    if (CHK_BIT(ts->vk_bitmap, BIT_MENU)) input_report_key_oppo(ts->kpd_input_dev, KEY_MENU, 0);
    if (CHK_BIT(ts->vk_bitmap, BIT_HOME)) input_report_key_oppo(ts->kpd_input_dev, KEY_HOMEPAGE, 0);
    if (CHK_BIT(ts->vk_bitmap, BIT_BACK)) input_report_key_oppo(ts->kpd_input_dev, KEY_BACK, 0);
...
static void tp_work_func(struct touchpanel_data* ts) {
     ...
     *  1.IRQ_EXCEPTION /IRQ_GESTURE /IRQ_IGNORE /IRQ_FW_CONFIG --->should be only reported
     * individually 2.IRQ_TOUCH && IRQ_BTN_KEY --->should depends on real situation && set
     * correspond bit on trigger_reason
     */
...
static void tp_fw_update_work(struct work_struct* work) {
    ...
    struct touchpanel_data* ts = container_of(work, struct touchpanel_data, fw_update_work);
    ...
    if (ts->ts_ops->fw_update) {
        ...
        if (!ret || ts->is_noflash_ic) {
            do {
                ...
                if (!ts->is_noflash_ic) {  // noflash update fw in reset and do bootloader reset in
                                           // get_chip_info
...
static irqreturn_t tp_irq_thread_fn(int irq, void* dev_id) {
    ...
    if (ts->irq_need_dev_resume_ok) {
        if (ts->i2c_ready == false) {
            ...
            wait_event_interruptible_timeout(waiter, ts->i2c_ready, msecs_to_jiffies(50));
...
static void tp_freq_hop_work(struct work_struct* work) {
    struct touchpanel_data* ts =
            container_of(work, struct touchpanel_data, freq_hop_info.freq_hop_work.work);
    ...
    if (ts->freq_hop_info.freq_hop_simulating) {
        ...
        queue_delayed_work(ts->freq_hop_info.freq_hop_workqueue, &ts->freq_hop_info.freq_hop_work,
...
int tp_gesture_enable_flag(void) {
    if (!g_tp || !g_tp->is_incell_panel) return LCD_POWER_OFF;
...
static ssize_t proc_gesture_control_read(struct file* file, char __user* user_buf, size_t count,
                                         loff_t* ppos) {
    int ret = 0;
...
static ssize_t proc_coordinate_read(struct file* file, char __user* user_buf, size_t count,
                                    loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    ...
    ret = snprintf(page, PAGESIZE - 1, "%u,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%u",
                   ts->gesture.gesture_type, ts->gesture.Point_start.x, ts->gesture.Point_start.y,
                   ts->gesture.Point_end.x, ts->gesture.Point_end.y, ts->gesture.Point_1st.x,
                   ts->gesture.Point_1st.y, ts->gesture.Point_2nd.x, ts->gesture.Point_2nd.y,
...
static ssize_t proc_ps_support_read(struct file* file, char __user* user_buf, size_t count,
                                    loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_game_switch_write(struct file* file, const char __user* buffer, size_t count,
                                      loff_t* ppos) {
    int value = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_game_switch_read(struct file* file, char __user* user_buf, size_t count,
                                     loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_black_screen_test_read(struct file* file, char __user* user_buf, size_t count,
                                           loff_t* ppos) {
    int ret = 0;
    int retry = 20;
    int msg_size = 256;
    ...
    if (!ts || !ts->gesture_test.flag) return 0;
...
OUT:
    ...
    if (!ts->auto_test_force_pass_support) {
        ret = simple_read_from_buffer(user_buf, count, ppos, ts->gesture_test.message,
...
static ssize_t proc_black_screen_test_write(struct file* file, const char __user* buffer,
                                            size_t count, loff_t* ppos) {
    int value = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));

    if (count > 2 || !ts) return count;
...
static ssize_t proc_get_irq_depth_read(struct file* file, char __user* user_buf, size_t count,
                                       loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_irq_status_write(struct file* file, const char __user* user_buf, size_t count,
                                     loff_t* ppos) {
    int value = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_glove_control_write(struct file* file, const char __user* buffer, size_t count,
                                        loff_t* ppos) {
    int ret = 0;
    char buf[3] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_glove_control_read(struct file* file, char __user* user_buf, size_t count,
                                       loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t cap_vk_show(struct kobject* kobj, struct kobj_attribute* attr, char* buf) {
    ...
    if (!g_tp) return sprintf(buf, "not support");
...
static ssize_t proc_debug_control_read(struct file* file, char __user* buf, size_t count,
                                       loff_t* ppos) {
    uint8_t ret = 0;
    char page[PAGESIZE] = {0};
...
static ssize_t proc_debug_control_write(struct file* file, const char __user* buf, size_t count,
                                        loff_t* lo) {
    int tmp = 0;
    char buffer[4] = {0};
...
static ssize_t proc_limit_area_read(struct file* file, char __user* user_buf, size_t count,
                                    loff_t* ppos) {
    ssize_t ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    ...
    ret = snprintf(page, PAGESIZE - 1,
                   "limit_area = %d left_x1 = %d right_x1 = %d left_x2 = %d right_x2 = %d left_x3 "
                   "= %d right_x3 = %d left_y1 = %d right_y1 = %d left_y2 = %d right_y2 = %d "
                   "left_y3 = %d right_y3 = %d\n",
                   ts->edge_limit.limit_area, ts->edge_limit.left_x1, ts->edge_limit.right_x1,
                   ts->edge_limit.left_x2, ts->edge_limit.right_x2, ts->edge_limit.left_x3,
                   ts->edge_limit.right_x3, ts->edge_limit.left_y1, ts->edge_limit.right_y1,
                   ts->edge_limit.left_y2, ts->edge_limit.right_y2, ts->edge_limit.left_y3,
...
static ssize_t proc_limit_area_write(struct file* file, const char __user* buffer, size_t count,
                                     loff_t* ppos) {
    char buf[8];
    int temp;
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    ...
    sscanf(buf, "%d", &temp);
    if (temp < 0 || temp > 10) return count;
    ...
    TPD_INFO(
            "limit_area = %d; left_y1 = %d; right_y1 = %d; left_y2 = %d; right_y2 = %d; left_y3 = "
            "%d; right_y3 = %d\n",
...
static ssize_t proc_limit_control_read(struct file* file, char __user* user_buf, size_t count,
                                       loff_t* ppos) {
    ssize_t ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_limit_control_write(struct file* file, const char __user* buffer, size_t count,
                                        loff_t* ppos) {
    char buf[8] = {0};
    int ret = 0, temp = 0;
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_dir_control_read(struct file* file, char __user* user_buf, size_t count,
                                     loff_t* ppos) {
    ssize_t ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));

    if (!ts || !ts->ts_ops->get_touch_direction) return 0;
...
static ssize_t proc_dir_control_write(struct file* file, const char __user* buffer, size_t count,
                                      loff_t* ppos) {
    char buf[8] = {0};
    int temp = 0;
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    ...
    if (ts->default_hor_area) {  // if configed
        ...
        TPD_INFO(
                "limit_area = %d; left_x1 = %d; right_x1 = %d; left_x2 = %d; right_x2 = %d; "
                "left_x3 = %d; right_x3 = %d\n",
                ts->edge_limit.limit_area, ts->edge_limit.left_x1, ts->edge_limit.right_x1,
                ts->edge_limit.left_x2, ts->edge_limit.right_x2, ts->edge_limit.left_x3,
                ts->edge_limit.right_x3);
        ...
        TPD_INFO(
                "limit_area = %d; left_y1 = %d; right_y1 = %d; left_y2 = %d; right_y2 = %d; "
                "left_y3 = %d; right_y3 = %d\n",
                ts->edge_limit.limit_area, ts->edge_limit.left_y1, ts->edge_limit.right_y1,
                ts->edge_limit.left_y2, ts->edge_limit.right_y2, ts->edge_limit.left_y3,
                ts->edge_limit.right_y3);
...
static ssize_t proc_limit_valid_read(struct file* file, char __user* user_buf, size_t count,
                                     loff_t* ppos) {
    ssize_t ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_limit_valid_write(struct file* file, const char __user* buffer, size_t count,
                                      loff_t* ppos) {
    char buf[8] = {0};
    int temp = 0;
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_noise_modetest_read(struct file* file, char __user* user_buf, size_t count,
                                        loff_t* ppos) {
    ssize_t ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));

    if (!ts || !ts->ts_ops->get_noise_modetest) return 0;
...
static ssize_t proc_noise_modetest_write(struct file* file, const char __user* buffer, size_t count,
                                         loff_t* ppos) {
    char buf[8] = {0};
    int temp = 0;
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_fw_update_write(struct file* file, const char __user* page, size_t size,
                                    loff_t* lo) {
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    int val = 0;
    int ret = 0;
    ...
    if (!ts->force_update && ts->firmware_update_type != 2) ts->force_update = !!val;
...
static ssize_t proc_aging_test_read(struct file* file, char __user* user_buf, size_t count,
                                    loff_t* ppos) {
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    uint8_t ret = 0;
    char page[PAGESIZE] = {0};
    ...
    if (!ret) TPD_INFO("%s:no support aging_test2\n", __func__);
    ret = snprintf(page, PAGESIZE - 1, "%d, %d\n", ret, ts->aging_test);
    ret = simple_read_from_buffer(user_buf, count, ppos, page, strlen(page));
    return ret;
}

static ssize_t proc_aging_test_write(struct file* file, const char __user* page, size_t size,
                                     loff_t* lo) {
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    int val = 0;
    char buf[4] = {0};
    if (!ts) return size;
    if (!ts->aging_test_ops) return size;
    if ((!ts->aging_test_ops->start_aging_test) || (!ts->aging_test_ops->finish_aging_test))
        return size;
    if (size > 2) return size;
    if (copy_from_user(buf, page, size)) {
...
static ssize_t proc_finger_protect_result_read(struct file* file, char __user* user_buf,
                                               size_t count, loff_t* ppos) {
    uint8_t ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_finger_protect_trigger_write(struct file* file, const char __user* buffer,
                                                 size_t count, loff_t* ppos) {
    int op = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_register_info_read(struct file* file, char __user* user_buf, size_t count,
                                       loff_t* ppos) {
    int ret = 0;
    int i = 0;
    ...
    if (ts->ts_ops->register_info_read) {
        ...
        for (i = 0; i < ts->reg_info.reg_length; i++) {
            num_read_chars +=
                    snprintf(&(page[num_read_chars]), PAGESIZE - 1, "reg_addr(0x%x) = 0x%x\n",
...
static ssize_t proc_register_info_write(struct file* file, const char __user* buffer, size_t count,
                                        loff_t* ppos) {
    int addr = 0, length = 0;
    char buf[16] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_incell_panel_info_read(struct file* file, char __user* user_buf, size_t count,
                                           loff_t* ppos) {
    uint8_t ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_report_point_first_read(struct file* file, char __user* user_buf, size_t count,
                                            loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_report_point_first_write(struct file* file, const char __user* buffer,
                                             size_t count, loff_t* ppos) {
    int value = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_rate_white_list_write(struct file* file, const char __user* buffer,
                                          size_t count, loff_t* ppos) {
    int value = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_rate_white_list_read(struct file* file, char __user* user_buf, size_t count,
                                         loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_switch_usb_state_write(struct file* file, const char __user* buffer,
                                           size_t count, loff_t* ppos) {
    int usb_state = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    ...
    if (ts->is_usb_checked != usb_state) {
        ...
        if (!ts->is_suspended && (ts->suspend_state == TP_SPEEDUP_RESUME_COMPLETE) &&
            ...
            if (ts->smooth_level_array_support && ts->smooth_level_charging_array_support &&
                ts->ts_ops->smooth_lv_set && ts->smooth_level_chosen) {
...
static ssize_t proc_switch_usb_state_read(struct file* file, char __user* user_buf, size_t count,
                                          loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_wireless_charge_detect_write(struct file* file, const char __user* buffer,
                                                 size_t count, loff_t* ppos) {
...
static ssize_t proc_wireless_charge_detect_read(struct file* file, char __user* user_buf,
                                                size_t count, loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_smooth_level_write(struct file* file, const char __user* buffer, size_t count,
                                       loff_t* ppos) {
...
static ssize_t proc_smooth_level_read(struct file* file, char __user* user_buf, size_t count,
                                      loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_sensitive_level_write(struct file* file, const char __user* buffer,
                                          size_t count, loff_t* ppos) {
    int value = 0, raw_level = 0;
    char buf[5] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_sensitive_level_read(struct file* file, char __user* user_buf, size_t count,
                                         loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_optimized_time_write(struct file* file, const char __user* buffer, size_t count,
                                         loff_t* ppos) {
    int value = 0;
    char buf[5] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_optimized_time_read(struct file* file, char __user* user_buf, size_t count,
                                        loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_curved_size_read(struct file* file, char __user* user_buf, size_t count,
                                     loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_kernel_grip_write(struct file* file, const char __user* buffer, size_t count,
                                      loff_t* ppos) {
    int value = -1;
    char buf[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_kernel_grip_read(struct file* file, char __user* user_buf, size_t count,
                                     loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static int init_touchpanel_proc(struct touchpanel_data* ts) {
    ...
    if (ts->fw_update_app_support) {
        ...
        register_device_proc("tp", ts->panel_data.manufacture_info.version,
    ...
    if (ts->edge_limit_support || ts->fw_edge_limit_support) {
        prEntry_tmp = proc_create_data("oplus_tp_limit_enable", 0664, prEntry_tp,
        ...
        prEntry_tmp =
                proc_create_data("oplus_tp_direction", 0666, prEntry_tp, &touch_dir_proc_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }

        prEntry_tmp = proc_create_data("oplus_tp_limit_whitelist", 0666, prEntry_tp,
        ...
        if (ts->edge_limit_support) {
            prEntry_tmp = proc_create_data("oppo_tp_limit_area", 0664, prEntry_tp,
                                           &proc_limit_area_ops, ts);
            if (prEntry_tmp == NULL) {
                ret = -ENOMEM;
                TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
            }
    ...
    if (ts->glove_mode_support) {
        prEntry_tmp = proc_create_data("glove_mode_enable", 0666, prEntry_tp,
    ...
    if (ts->spurious_fp_support) {
        prEntry_tmp = proc_create_data("finger_protect_result", 0666, prEntry_tp,
                                       &proc_finger_protect_result, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
        prEntry_tmp = proc_create_data("finger_protect_trigger", 0666, prEntry_tp,
                                       &proc_finger_protect_trigger, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
    ...
    prEntry_tmp =
            proc_create_data("oplus_register_info", 0664, prEntry_tp, &proc_register_info_fops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
    }
    ...
    if (ts->is_incell_panel) {
        prEntry_tmp =
                proc_create_data("incell_panel", 0664, prEntry_tp, &proc_incell_panel_fops, ts);
    }

    if (ts->gesture_test_support) {
        prEntry_tmp = proc_create_data("black_screen_test", 0666, prEntry_tp,
    ...
    if (ts->game_switch_support) {
        prEntry_tmp = proc_create_data("game_switch_enable", 0666, prEntry_tp,
    ...
    if (ts->noise_modetest_support) {
        prEntry_tmp = proc_create_data("oppo_tp_noise_modetest", 0664, prEntry_tp,
    ...
    if (ts->report_rate_white_list_support) {
        prEntry_tmp = proc_create_data("report_rate_white_list", 0666, prEntry_tp,
    ...
    if (ts->charger_pump_support) {
        prEntry_tmp = proc_create_data("charge_detect", 0666, prEntry_tp,
    ...
    if (ts->wireless_charger_support) {
        prEntry_tmp = proc_create_data("wireless_charge_detect", 0666, prEntry_tp,
    ...
    if (ts->smooth_level_support || ts->smooth_level_array_support) {
        prEntry_tmp =
                proc_create_data("smooth_level", 0666, prEntry_tp, &proc_smooth_level_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
    ...
    if (ts->sensitive_level_array_support) {
        prEntry_tmp = proc_create_data("sensitive_level", 0666, prEntry_tp,
    ...
    if (ts->optimized_show_support) {
        prEntry_tmp = proc_create_data("oplus_optimized_time", 0666, prEntry_tp,
    ...
    if (ts->auto_test_need_cal_support) {
        ...
        prEntry_tmp =
                proc_create_data("calibration_status", 0666, prEntry_tp, &proc_cal_status_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
    ...
    if (ts->kernel_grip_support) {
        prEntry_tmp = proc_create_data("kernel_grip_default_para", 0664, prEntry_tp,
                                       &tp_grip_default_para_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
...
static ssize_t proc_freq_hop_write(struct file* file, const char __user* buffer, size_t count,
                                   loff_t* ppos) {
    int value = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_freq_hop_read(struct file* file, char __user* user_buf, size_t count,
                                  loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_earsense_palm_control_write(struct file* file, const char __user* buffer,
                                                size_t count, loff_t* ppos) {
    int value = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    ...
    if (value == ts->palm_enable) return count;
...
static ssize_t proc_earsense_palm_control_read(struct file* file, char __user* user_buf,
                                               size_t count, loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_earsense_es_enable_write(struct file* file, const char __user* buffer,
                                             size_t count, loff_t* ppos) {
    int value = 0;
    ...
    if (value == ts->es_enable) return count;

    mutex_lock(&ts->mutex);
    if ((ts->es_enable != 1) && (value != 1)) state_changed = false;
    ts->es_enable = value;
    if (!ts->es_enable) {
...
static ssize_t proc_earsense_es_enable_read(struct file* file, char __user* user_buf, size_t count,
                                            loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_earsense_touchcnt_read(struct file* file, char __user* user_buf, size_t count,
                                           loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_earsense_rawdata_read(struct file* file, char __user* user_buf, size_t count,
                                          loff_t* ppos) {
    int ret = 0;
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    int read_len = 0;
    ...
    if (ts->delta_state == TYPE_DELTA_IDLE) {
        ...
        if (ret) TPD_INFO("touch rawdata read fail\n");
        kfree(tmp_data);
        *ppos += 11;
    } else {
        mutex_unlock(&ts->mutex);
...
static ssize_t proc_earsense_delta_read(struct file* file, char __user* user_buf, size_t count,
                                        loff_t* ppos) {
    int ret = 0;
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    int read_len = 0;
    ...
    if (ret) TPD_INFO("tp rawdata read fail\n");
    *ppos += read_len;
    return read_len;
}
...
static ssize_t proc_earsense_selfdata_read(struct file* file, char __user* user_buf, size_t count,
                                           loff_t* ppos) {
    int ret = 0;
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    uint16_t data_len = 0;
    ...
    if (ts->delta_state == TYPE_DELTA_IDLE) {
        ...
        if (ret) TPD_INFO("tp self delta read fail\n");
        kfree(tmp_data);
        *ppos += 11;
    } else {
        mutex_unlock(&ts->mutex);
...
// write function of /proc/touchpanel/earsense/es_enable
static ssize_t proc_fd_enable_write(struct file* file, const char __user* buffer, size_t count,
                                    loff_t* ppos) {
    int value = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    ...
    if (value == ts->fd_enable) return count;
...
static ssize_t proc_fd_enable_read(struct file* file, char __user* user_buf, size_t count,
                                   loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_fp_enable_write(struct file* file, const char __user* buffer, size_t count,
                                    loff_t* ppos) {
    int value = 0;
    char buf[4] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    ...
    if (value == ts->fp_enable) return count;

    mutex_lock(&ts->mutex);
    ts->fp_enable = value;
    if (!ts->fp_enable) ts->fp_info.touch_state = 0;  // reset touch state
    if (!ts->is_suspended && (ts->suspend_state == TP_SPEEDUP_RESUME_COMPLETE)) {
        ts->ts_ops->enable_fingerprint(ts->chip_data, !!ts->fp_enable);
    } else if (ts->is_suspended) {
...
static ssize_t proc_fp_enable_read(struct file* file, char __user* user_buf, size_t count,
                                   loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_event_num_read(struct file* file, char __user* user_buf, size_t count,
                                   loff_t* ppos) {
    int ret = 0;
...
static ssize_t proc_fd_touch_read(struct file* file, char __user* user_buf, size_t count,
                                  loff_t* ppos) {
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
...
static int tp_health_monitor_read_func(struct seq_file* s, void* v) {
    ...
    if (ts->health_monitor_v2_support) {
        if (mon_data_v2->fw_version) {
            ...
            strncpy(mon_data_v2->fw_version, ts->panel_data.manufacture_info.version,
        ...
        seq_printf(s,
                   ...
                   ts->panel_data.manufacture_info.manufacture, mon_data->bootup_test,
                   mon_data->repeat_finger, mon_data->miss_irq, mon_data->grip_report,
                   mon_data->baseline_err, mon_data->noise_count, mon_data->shield_palm,
                   mon_data->shield_edge, mon_data->shield_metal, mon_data->shield_water,
                   mon_data->shield_esd, mon_data->hard_rst, mon_data->inst_rst,
                   mon_data->parity_rst, mon_data->wd_rst, mon_data->other_rst, mon_data->reserve1,
                   mon_data->reserve2, mon_data->reserve3, mon_data->reserve4, mon_data->reserve5,
...
static ssize_t health_monitor_control(struct file* file, const char __user* buf, size_t count,
                                      loff_t* lo) {
    struct touchpanel_data* ts = PDE_DATA(file_inode(file));
    char buffer[4] = {0};
    int tmp = 0;
...
static int init_debug_info_proc(struct touchpanel_data* ts) {
    ...
    prEntry_tmp =
            proc_create_data("data_limit", 0666, prEntry_debug_info, &tp_limit_data_proc_fops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
    }
    ...
    prEntry_tmp =
            proc_create_data("baseline", 0666, prEntry_debug_info, &tp_baseline_data_proc_fops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
    }
    ...
    prEntry_tmp =
            proc_create_data("self_raw", 0666, prEntry_debug_info, &tp_self_raw_data_proc_fops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
    }
    ...
    prEntry_tmp = proc_create_data("main_register", 0666, prEntry_debug_info,
    ...
    prEntry_tmp =
            proc_create_data("abs_doze", 0666, prEntry_debug_info, &tp_abs_doze_proc_fops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
    }
    ...
    if (ts->health_monitor_support || ts->health_monitor_v2_support) {
        ...
        prEntry_tmp = proc_create_data("health_monitor", 0666, prEntry_debug_info,
                                       &tp_health_monitor_proc_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
    ...
    if (ts->freq_hop_simulate_support) {
        ...
        prEntry_tmp = proc_create_data("freq_hop_simulate", 0666, prEntry_debug_info,
    ...
    if (ts->ear_sense_support) {
        ...
        prEntry_tmp =
                proc_create_data("rawdata", 0666, prEntry_earsense, &tp_earsense_rawdata_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
        ...
        prEntry_tmp =
                proc_create_data("delta", 0666, prEntry_earsense, &tp_earsense_delta_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
        // show self delta for earsense
        prEntry_tmp = proc_create_data("hover_selfdata", 0666, prEntry_earsense,
        ...
        prEntry_tmp = proc_create_data("palm_control", 0666, prEntry_earsense,
        ...
        prEntry_tmp = proc_create_data("es_enable", 0666, prEntry_earsense,
        ...
        prEntry_tmp = proc_create_data("es_touch_count", 0666, prEntry_earsense,
                                       &tp_earsense_es_touchcnt_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
    ...
    if (ts->face_detect_support) {
        ...
        prEntry_tmp =
                proc_create_data("fd_touch_count", 0666, ts->prEntry_tp, &fd_touch_num_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
    ...
    if (ts->report_point_first_support) {
        prEntry_tmp = proc_create_data("report_point_first", 0666, ts->prEntry_tp,
                                       &report_point_first_proc_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
...
static int init_input_device(struct touchpanel_data* ts) {
    ...
    switch (ts->vk_type) {
        case TYPE_PROPERTIES: {
            ...
            if (!vk_properties_kobj || ret) TPD_DEBUG("failed to create board_properties\n");
            break;
        }
        case TYPE_AREA_SEPRATE: {
            TPD_DEBUG("Type 2:using same IC (button zone &&  touch zone are seprate)\n");
            if (CHK_BIT(ts->vk_bitmap, BIT_MENU)) set_bit(KEY_MENU, ts->kpd_input_dev->keybit);
            if (CHK_BIT(ts->vk_bitmap, BIT_HOME)) set_bit(KEY_HOMEPAGE, ts->kpd_input_dev->keybit);
            if (CHK_BIT(ts->vk_bitmap, BIT_BACK)) set_bit(KEY_BACK, ts->kpd_input_dev->keybit);
            break;
        }
        default:
            break;
...
static int init_parse_dts(struct device* dev, struct touchpanel_data* ts) {
    ...
    if (ts->panel_data.project_num > 0) {
        ...
        for (i = 0; i < ts->panel_data.project_num; i++) {
            ...
            if (ts->panel_data.platform_support_commandline[i] == NULL ||
                ts->panel_data.platform_support_external_name[i] == NULL) {
                TPD_INFO(
                        "panel_data.platform_support_commandline or platform_support_external_name "
                        "kzalloc error\n");
            ...
            rc = of_property_read_string_index(
                    ...
                    (const char**)&ts->panel_data.platform_support_external_name[i]);
            if (rc) {
    ...
    ts->report_point_first_support =
            of_property_read_bool(ts->dev->of_node, "report_point_first_support");
    ts->lcd_wait_tp_resume_finished_support =
            of_property_read_bool(np, "lcd_wait_tp_resume_finished_support");
    ...
    if (gpio_is_valid(ts->hw_res.reset_gpio)) {
        ...
        if (rc) TPD_INFO("unable to request gpio [%d]\n", ts->hw_res.reset_gpio);
    } else {
    ...
    if (rc) {
        ...
        if (ts->firmware_in_dts != NULL) {
            ...
            if (ts->firmware_in_dts->data == NULL) {
                ...
                rc = of_property_read_u8_array(np, "firmware-data", (u8*)ts->firmware_in_dts->data,
                                               val);
                if (rc) {
    ...
    TPD_INFO(
            "LCD_WIDTH = %d, LCD_HEIGHT = %d, max_x = %d, max_y = %d, limit_witdh = %d, "
            "limit_height = %d\n",
            ts->resolution_info.LCD_WIDTH, ts->resolution_info.LCD_HEIGHT,
            ts->resolution_info.max_x, ts->resolution_info.max_y, ts->touch_major_limit.width_range,
    ...
    if (ts->health_monitor_support) {
        ...
        TPD_INFO("eli_size = %d, eli_ver_range = %d, eli_hor_range = %d\n",
                 ts->monitor_data.eli_size, ts->monitor_data.eli_ver_range,
    ...
    rc = of_property_read_u32_array(np, "touchpanel,smooth-level-charging", temp_array,
                                    SMOOTH_LEVEL_NUM);
    if (rc) {
    ...
    rc = of_property_read_u32(ts->dev->of_node, "touchpanel,default_hor_area",
                              &ts->default_hor_area);
    if (rc) {
    ...
    // set incell panel parameter, for of_property_read_bool return 1 when success and return 0 when
    // item is not exist
    ...
    if (IS_ERR_OR_NULL(ts->hw_res.vdd_2v8)) {
        ...
        if (regulator_count_voltages(ts->hw_res.vdd_2v8) > 0) {
            ...
            if (ts->hw_res.vdd_volt) {
                ret = regulator_set_voltage(ts->hw_res.vdd_2v8, ts->hw_res.vdd_volt,
                                            ts->hw_res.vdd_volt);
            } else {
...
static void esd_handle_func(struct work_struct* work) {
    int ret = 0;
    struct touchpanel_data* ts =
            container_of(work, struct touchpanel_data, esd_info.esd_check_work.work);
        ...
        if (ts->irq <= 0) {
            ...
            if (ts->is_noflash_ic) {
                ...
                TPD_DEBUG("%s, [irq info] irq_gpio is %d, ts->s_client->irq is %d\n", __func__,
                          ts->hw_res.irq_gpio, ts->s_client->irq);
            } else {
                ts->client->irq = gpio_to_irq(ts->hw_res.irq_gpio);
                TPD_DEBUG("%s, [irq info] irq_gpio is %d, ts->client->irq is %d\n", __func__,
                          ts->hw_res.irq_gpio, ts->client->irq);
            }
        }

        ret = request_threaded_irq(ts->irq, NULL, tp_irq_thread_fn, ts->irq_flags | IRQF_ONESHOT,
                                   TPD_DEVICE, ts);
        if (ret < 0) {
...
static void touch_read_delta(struct work_struct* work) {
    ...
    mutex_lock(&ts->mutex);
    if (!ts->is_suspended) {
        ts->earsense_ops->delta_read(ts->chip_data, ts->earsense_delta,
                                     2 * ts->hw_res.EARSENSE_TX_NUM * ts->hw_res.EARSENSE_RX_NUM);
    }
    mutex_unlock(&ts->mutex);
    mutex_unlock(&ts->mutex_earsense);
...
int register_common_touch_device(struct touchpanel_data* pdata) {
    ...
    if (ts->ear_sense_support) {
        ...
        ts->earsense_delta =
                kzalloc(2 * ts->hw_res.EARSENSE_TX_NUM * ts->hw_res.EARSENSE_RX_NUM, GFP_KERNEL);
    ...
    if (ts->health_monitor_support) {
        ts->monitor_data.eli_ver_pos = kzalloc(
                (ts->monitor_data.eli_ver_range / ts->monitor_data.eli_size) *
                        (ts->resolution_info.max_y / ts->monitor_data.eli_size) * sizeof(int),
                GFP_KERNEL);
        ts->monitor_data.eli_hor_pos = kzalloc(
                (ts->monitor_data.eli_hor_range / ts->monitor_data.eli_size) *
                        (ts->resolution_info.max_x / ts->monitor_data.eli_size) * sizeof(int),
                GFP_KERNEL);
...
power_control_failed:
    ...
    if (gpio_is_valid(ts->hw_res.enable2v8_gpio)) gpio_free(ts->hw_res.enable2v8_gpio);

    if (gpio_is_valid(ts->hw_res.enable1v8_gpio)) gpio_free(ts->hw_res.enable1v8_gpio);
...
static int tp_suspend(struct device* dev) {
    ...
    TPD_INFO(
            "tp_suspend ts->spuri_fp_touch.fp_trigger =%d  ts->i2c_ready =%d  "
            "ts->spuri_fp_touch.lcd_resume_ok=%d \n",
    ...
    if (!ts->is_incell_panel || (ts->black_gesture_support && ts->gesture_enable > 0)) {
        ...
        if (ts->game_switch_support) ts->ts_ops->mode_switch(ts->chip_data, MODE_GAME, false);
...
static void tp_resume(struct device* dev) {
    ...
    if (ts->loading_fw) goto NO_NEED_RESUME;
...
static void lcd_trigger_load_tp_fw(struct work_struct* work) {
    struct touchpanel_data* ts =
            container_of(work, struct touchpanel_data, lcd_trigger_load_tp_fw_work);
...
static void lcd_tp_refresh_work(struct work_struct* work) {
    struct touchpanel_data* ts = container_of(work, struct touchpanel_data, tp_refresh_work);
...
static void speedup_resume(struct work_struct* work) {
    int timed_out = 0;
    u64 start_time = 0;
    struct touchpanel_data* ts = container_of(work, struct touchpanel_data, speed_up_work);
    ...
    if (ts->use_resume_notify && (!ts->fp_info.touch_state || !ts->report_flow_unlock_support)) {
        timed_out = wait_for_completion_timeout(&ts->resume_complete,
                                                1 * HZ);  // wait resume over for 1s
...
#endif
                timed_out = wait_for_completion_timeout(&ts->pm_complete,
