...
#include "oplus_touchscreen/Synaptics/S3706/synaptics_s3706.h"
...
struct tp_dev_name tp_dev_names[] = {
        {TP_OFILM, "OFILM"},     {TP_BIEL, "BIEL"},     {TP_TRULY, "TRULY"},
        {TP_BOE, "BOE"},         {TP_G2Y, "G2Y"},       {TP_TPK, "TPK"},
        {TP_JDI, "JDI"},         {TP_TIANMA, "TIANMA"}, {TP_SAMSUNG, "SAMSUNG"},
        {TP_DSJM, "DSJM"},       {TP_BOE_B8, "BOEB8"},  {TP_INNOLUX, "INNOLUX"},
        {TP_HIMAX_DPT, "DPT"},   {TP_AUO, "AUO"},       {TP_DEPUTE, "DEPUTE"},
        {TP_HUAXING, "HUAXING"}, {TP_HLT, "HLT"},       {TP_DJN, "DJN"},
        {TP_UNKNOWN, "UNKNOWN"},
};

#define GET_TP_DEV_NAME(tp_type) \
    ((tp_dev_names[tp_type].type == (tp_type)) ? tp_dev_names[tp_type].name : "UNMATCH")
...
bool __init tp_judge_ic_match(char* tp_ic_name) {
    ...
    switch (get_project()) {
        case 18081:
        case 18085:
            pr_info("[TP] tp judge ic forward for 18081\n");
...
int tp_util_get_vendor(struct hw_resource* hw_res, struct panel_info* panel_data) {
    ...
    snprintf(panel_data->fw_name, MAX_FW_NAME_LENGTH, "tp/%d/FW_%s_%s.img", prj_id,
             panel_data->chip_name, vendor);

    if (panel_data->test_limit_name) {
        snprintf(panel_data->test_limit_name, MAX_LIMIT_DATA_LENGTH, "tp/%d/LIMIT_%s_%s.img",
...
int reconfig_power_control(struct touchpanel_data* ts) {
    int ret = 0;
    if ((is_project(OPPO_18081) || is_project(OPPO_18097) || is_project(OPPO_18099))) {
        if (get_PCB_Version() == HW_VERSION__10) {
